...
// MI VISUAL - Biblioteca v53 fix Jefatura
...
function recurso(row){const nombre=row.nombre.replace(/"/g,'').trim(),link=row.link.replace(/"/g,'').trim();if(!nombre||!link||!validLink(link))return null;return {id:row.id_legacy,nombre,link,categoria:categoriaBiblioteca(nombre)}}
...
Deno.serve(async req=>{
 ...
 const {data:permission,error:permissionError}=await admin.from("app_permissions").select("activo,ver,alcance_datos").eq("perfil",user.perfil).eq("modulo","BIBLIOTECA").maybeSingle();
 if(permissionError)throw Error("permission query");
 if(!permission?.activo||!permission.ver||normalizarDestinoAcceso(permission.alcance_datos)==="SIN ACCESO")return respond({ok:false,error:"Sin permiso para Biblioteca."},403);
 const {data:rows,error:rowsError}=await admin.from("biblioteca_catalogo_migracion").select("source_row,id_legacy,nombre,link").order("source_row");
 if(rowsError)throw Error("catalog query");
 const recursos=(rows||[]).map(recurso).filter(Boolean);
 return respond({ok:true,version:"V1-BIBLIOTECA-PILOT",perfil:user.perfil,recursos});
 }catch{return respond({ok:false,error:"No se pudo consultar Biblioteca. Reintenta."},500)}
});
