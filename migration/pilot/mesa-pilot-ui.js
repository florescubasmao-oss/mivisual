...
function crPerfil(){return (window.mesaUser?.perfil||"").toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");}
function crEsGerenciaLima(){return crPerfil().trim()==="GERENCIA LIMA";}
function crEsGerenciaGeneral(){return ["GERENCIA GENERAL","GERENCIAL GENERAL"].includes(crPerfil().trim());}
function crEsGerenciaSoloLectura(){return crEsGerenciaLima()||crEsGerenciaGeneral();}
...
async function crRegistrar(){
  ...
  try{
    const especial=crEsCasoProduccion();
    const payload={accion:"registrarConsultaReclamo",usuario:window.mesaUser?.usuario,categoria:document.getElementById('crCategoria').value,subcategoria:document.getElementById('crSubcategoria').value,urgencia:document.getElementById('crUrgencia').value,descripcion:document.getElementById('crDescripcion').value,evidencias:document.getElementById('crEvidencias').value};
...
async function crDetalle(id){
  try{
    ...
    window.mesaVersions[id]=x.version;
    const esResponsable=x.puedeResolver===true;
    const puedeResolver=!finalizado&&esResponsable;
    const puedeRestablecer=!crEsGerenciaSoloLectura()&&esResponsable&&h.historial.some(y=>["CAMBIO DE ESTADO","COMENTARIO"].includes(String(y.accion||"").toUpperCase()));
    ...
    const controles=crEsGerenciaSoloLectura()
      ? `<div class="cr-final-note">Vista de solo lectura para Gerencia.</div>`
      : finalizado
...
async function crComentar(id){try{await crApi({accion:"agregarComentarioReclamo",usuario:window.mesaUser?.usuario,id,comentario:crComentario.value});crDetalle(id);}catch(e){alert(e.message);}}
async function crActualizar(id){
  try{
    ...
    await crApi({accion:"actualizarConsultaReclamo",usuario:window.mesaUser?.usuario,id,estado,comentario});
    crDetalle(id);
  }catch(e){alert(e.message);}
}
async function crRestablecer(id){
  try{
    ...
    await crApi({accion:"restablecerConsultaReclamo",usuario:window.mesaUser?.usuario,id,motivo});
