...
CREATE OR REPLACE FUNCTION public.mv_bono_sup_calcular_supervisor(p_periodo text, p_supervisor text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select
    coalesce(sum(c.produccion),0),
    coalesce(sum(c.finalizadas),0),
    coalesce(sum(c.total_gestionadas),0)
  into puntos,finalizadas,total_gestionadas
  from unnest(cuadrillas) q(cuadrilla)
  left join public.bono_supervisores_operativo_cache c
    on c.periodo=p and c.cuadrilla=q.cuadrilla;
  ...
  -- Calidad.
  select
    coalesce(sum(c.observaciones_win),0),
    coalesce(sum(c.observaciones_win_penalizadas),0),
    coalesce(sum(c.monto_penalizado_win),0),
    coalesce(sum(c.rojo_asignadas),0),
    coalesce(sum(c.recableados),0),
    coalesce(sum(c.total_gar_vtr),0),
    coalesce(sum(c.finalizadas_vtr),0)
  ...
  if rojo>0 then
    ...
    select coalesce(round(avg(c.recableado_pct),2),0)
    into rec_pct
    from public.bono_supervisores_operativo_cache c
    where c.periodo=p and c.cuadrilla=any(cuadrillas);
  end if;

  if vtr_fin>0 then
    vtr_pct:=round(vtr_inc::numeric/vtr_fin*100,2);
  else
    select coalesce(round(avg(c.vtrgar_pct),2),0)
    into vtr_pct
    from public.bono_supervisores_operativo_cache c
    where c.periodo=p and c.cuadrilla=any(cuadrillas);
  end if;
  ...
  select
    coalesce(sum(c.sla_instalaciones_total+c.sla_visitas_total),0),
    coalesce(sum(c.sla_instalaciones_cumplen_ajustado+c.sla_visitas_cumplen_ajustado),0),
    coalesce(sum(c.sla_cumplen_bruto),0),
    coalesce(sum(c.sla_total_finalizadas),0),
    coalesce(sum(c.sla_sin_tiempos),0),
    coalesce(sum(c.sla_sin_partida),0),
    coalesce(sum(c.sla_sin_parametro),0),
    coalesce(sum(c.sla_instalaciones_total),0),
    coalesce(sum(c.sla_instalaciones_cumplen_ajustado),0),
    coalesce(sum(c.sla_visitas_total),0),
    coalesce(sum(c.sla_visitas_cumplen_ajustado),0),
    coalesce(sum(c.sla_excepciones_aprobadas),0),
    coalesce(sum(c.sla_excepciones_pendientes),0),
    coalesce(sum(c.sla_excepciones_rechazadas),0)
  into
  ...
  select
    count(*)::int,
    coalesce(sum(least(100,greatest(0,ac.puntaje_cliente/20*100))),0),
    coalesce(sum(least(100,greatest(0,ac.puntaje_orden_limpieza/20*100))),0)
  into aud_count,suma_cliente,suma_orden
  from public.actividad_campo_migracion ac
  where to_char(ac.fecha,'YYYY-MM')=p
    and public.mv_bono_sup_usuario_key(ac.supervisor)=sup
    and public.mv_actividad_es_auditoria(ac.tipo_actividad)
    and ac.puntaje_cliente is not null
    and ac.puntaje_orden_limpieza is not null;
