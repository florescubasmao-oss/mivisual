...
-- Preview y guard de identidad/coexistencia para Actas vivas.

create or replace function public.mv_sync_preview_actas(p_run_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if not found then raise exception 'Run no encontrado'; end if;
  if r.modulo<>'ACTAS' then raise exception 'Run no pertenece a ACTAS'; end if;
  ...
  select count(*) filter(where a.legacy_id is not null),
         count(*) filter(where a.legacy_id is null)
  into v_existing,v_new
  from public.migration_sync_staging s
  left join public.actas_migracion a
    on a.legacy_id=trim(s.row_data->>'id')
  where s.run_id=p_run_id;

  select count(*) into v_nonlegacy
  from public.migration_sync_staging s
  ...
  where s.run_id=p_run_id
    and (
      a.archivo_storage_backend='SUPABASE_STORAGE'
      or nullif(trim(coalesce(a.archivo_storage_ref,'')),'') is not null
    );

  with src as (
    select
      s.source_key,
      trim(s.row_data->>'id') legacy_id,
      public.mv_actas_key(s.row_data->>'codigo_orden') orden_key,
      public.mv_actas_numero_key(s.row_data->>'numero_acta') numero_key
    from public.migration_sync_staging s
    where s.run_id=p_run_id
  ),
  conflicts as (
    ...
    from src
    join public.actas_migracion a
      on src.orden_key not in ('','0')
     and public.mv_actas_key(a.codigo_orden)=src.orden_key
     and coalesce(a.legacy_id,'')<>src.legacy_id
    union
    select src.source_key
    from src
    join public.actas_migracion a
      on src.numero_key<>''
     and public.mv_actas_numero_key(a.numero_acta)=src.numero_key
     and coalesce(a.legacy_id,'')<>src.legacy_id
  )
  select count(distinct source_key) into v_identity from conflicts;
  ...
  select count(distinct src.source_key) into v_drive
  from src
  join public.actas_migracion a
    on src.drive_id is not null
   and src.drive_id<>''
   and a.drive_file_id=src.drive_id
   and coalesce(a.legacy_id,'')<>src.legacy_id;

  v_can :=
    r.status='VALIDATED'
    and v_staged=r.source_rows
    and v_staged>0
    and v_invalid=0
    and v_nonlegacy=0
    and v_storage=0
  ...
  return jsonb_build_object(
    'ok',true,'modulo','ACTAS','runId',p_run_id,
    'status',r.status,'sourceRows',r.source_rows,'stagedRows',v_staged,
    'existentes',v_existing,'nuevas',v_new,
    'invalidas',v_invalid,
    'conflictosPostgresql',v_nonlegacy,
    'conflictosStorage',v_storage,
    'conflictosIdentidad',v_identity,
    'conflictosDrive',v_drive,
    'puedeAplicar',v_can
  );
end $$;

revoke all on function public.mv_sync_preview_actas(uuid)
from public,anon,authenticated;
grant execute on function public.mv_sync_preview_actas(uuid)
to service_role;
...
begin
  ...
  v_preview:=public.mv_sync_preview_actas(p_run_id);
  if not coalesce((v_preview->>'puedeAplicar')::boolean,false) then
    raise exception 'Preview ACTAS bloqueado: %',v_preview;
  end if;
...
revoke all on function public.mv_sync_apply_actas(uuid,text,text)
from public,anon,authenticated;
grant execute on function public.mv_sync_apply_actas(uuid,text,text)
to service_role;
