...
-- Integra Actividad en Campo con CHECKLIST_ALMACEN de forma atómica.
...
declare
  ...
  checklist_result jsonb:=null;
  checklist_id text:='';
  comentario_checklist text:='';
begin
  ...
  if tipo_actividad='CHECKLIST' then
    comentario_checklist:=trim(coalesce(
      p_data->'checklist'->>'comentarioFinal',
      p_data->>'comentarioFinal',
      ''
    ));
    if comentario_checklist='' then
      raise exception 'Debe ingresar el comentario final del checklist';
    end if;

    checklist_data:=coalesce(p_data->'checklist','{}'::jsonb)
      || jsonb_build_object(
        'cuadrilla',cuadrilla_in,
        'origenRegistro','ACTIVIDAD_CAMPO',
        'comentarioFinal',comentario_checklist
      );

    checklist_result:=public.mv_checklist_registrar(usuario_u,checklist_data);
    checklist_id:=coalesce(checklist_result->>'id','');
  end if;
  ...
  if checklist_id<>'' then
    observaciones_texto:=observaciones_texto||
      case when observaciones_texto='' then '' else E'\n' end||
      'ID CHECKLIST ALMACEN: '||checklist_id;
  end if;

  if tipo_actividad<>'CHECKLIST' then
    foto1:=coalesce(
      nullif(p_data->>'foto1Url',''),
      case when jsonb_typeof(p_data->'foto1')='string' then p_data->>'foto1' else '' end,
      ''
    );
    foto2:=coalesce(
      nullif(p_data->>'foto2Url',''),
      case when jsonb_typeof(p_data->'foto2')='string' then p_data->>'foto2' else '' end,
      ''
    );
    foto3:=coalesce(
      nullif(p_data->>'foto3Url',''),
      nullif(p_data->>'fotoActaUrl',''),
      case when jsonb_typeof(p_data->'foto3')='string' then p_data->>'foto3' else '' end,
      ''
    );
    foto4:=coalesce(
      nullif(p_data->>'foto4Url',''),
      case when jsonb_typeof(p_data->'foto4')='string' then p_data->>'foto4' else '' end,
      ''
    );
  end if;
  ...
  insert into public.actividad_campo_migracion(
    ...
    tipo_actividad,
    tipo_actividad,
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'clientePresente',p_data->>'cliente_presente',''),
      'Cliente presente'
    ),
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'dniValidado',p_data->>'dni_validado',''),
      'DNI validado'
    ),
    estado_instalacion,
    ...
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'reservaCable',p_data->>'reserva_cable',''),
      'Reserva de cable'
    ),
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'potenciaConforme',p_data->>'potencia_conforme',''),
      'Potencia conforme'
    ),
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'velocidadConforme',p_data->>'velocidad_conforme',''),
      'Velocidad conforme'
    ),
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'limpiezaTrabajo',p_data->>'limpieza_trabajo',''),
      'Limpieza del trabajo'
    ),
    public.mv_actividad_validar_si_no(
      coalesce(p_data->>'clienteConforme',p_data->>'cliente_conforme',''),
      'Cliente conforme'
    ),
    observaciones_texto,
    foto1,foto2,foto3,
  ...
  return jsonb_build_object(
    ...
    'checklistId',checklist_id,
    'puntajeTotal',
      case when puntajes is null
           then to_jsonb(''::text)
           else to_jsonb((puntajes->>'total')::numeric)
      end,
    'clasificacion',
      case when puntajes is null then '' else puntajes->>'clasificacion' end
  );
end;
$$;
