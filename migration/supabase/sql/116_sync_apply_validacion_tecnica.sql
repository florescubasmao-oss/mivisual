...
-- Aplicador específico, transaccional e idempotente para VALIDACION_TECNICA.
-- Solo consume runs VALIDATED del staging general.
...
begin
  ...
  if not found then
    raise exception 'Run de sincronización no existe';
  end if;
  if v_run.modulo <> 'VALIDACION_TECNICA' then
    raise exception 'Run pertenece a otro módulo: %', v_run.modulo;
  end if;
  if v_run.status <> 'VALIDATED' then
    raise exception 'Run debe estar VALIDATED. Estado actual: %', v_run.status;
  end if;
  ...
  set status='APPLIED',applied_rows=v_applied,applied_at=now(),
      notes=concat_ws(' | ',notes,'Aplicado VALIDACION_TECNICA por '||coalesce(p_actor,'SISTEMA'))
  where id=p_run_id;
  ...
  set postgres_rows=v_total,last_audit_at=now(),content_match=null,status='LIVE_VALIDAR',
      notes='Delta append aplicado desde staging; conteo PostgreSQL='||v_total||
            '. Aún se requiere resync completo de campos mutables antes del cutover.',
      updated_at=now()
...
revoke all on function public.mv_sync_apply_validacion_tecnica(uuid,text,text) from public,anon,authenticated;
grant execute on function public.mv_sync_apply_validacion_tecnica(uuid,text,text) to service_role;
