...
function ckLimpiarFiltrosVisuales(){['ckFiltroSede','ckFiltroTipo','ckFiltroCuadrilla','ckFiltroSupervisor','ckFiltroDesde','ckFiltroHasta','ckFiltroEstado','ckFiltroVencimiento','ckFiltroHerramienta'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});ckAplicarFiltrosVisuales();}
async function ckCargarLista(){const c=document.getElementById('ckContenido')||document.getElementById('ckLista');if(!c)return;try{const d=await ckApi({accion:'listarChecklistAlmacen',usuario:ckUser().usuario});const arr=d.checklist||[];CK_LISTA_ACTUAL=arr;const u=ckUser(),toolbar=u.perfil!=='TECNICO'?`<div class="ck-toolbar"><button class="ck-btn blue" onclick="ckAbrirInformeChecklist()">📥 Descargar informe de checklist</button></div>`:'';if(u.perfil==='TECNICO'){c.innerHTML=toolbar+ckKpisHtml(ckConteosLista(arr),false)+(arr.map(x=>ckCard(x,u)).join('')||'<div class="ck-card">No hay registros.</div>');}else{c.innerHTML=toolbar+ckRenderFiltrosRolV141(arr,u)+`<div id="ckResumenGeneralJefatura">${ckKpisHtml(ckConteosLista(arr),false)}</div><div id="ckResultadosJefatura">${ckRenderAgrupadoPorSede(arr)}</div>`;ckActivarDesplegablesSede();}}catch(e){c.innerHTML=`<div class="ck-card">${ckEsc(e.message)}</div>`}}
