...
async function ckRecolectarHerramientas(){const out=[];for(const box of document.querySelectorAll('#ckHerramientasLista .ck-detail-box')){const nombre=box.querySelector('[data-h-nombre]')?.value.trim(),serie=box.querySelector('[data-h-serie]')?.value.trim(),estado=box.querySelector('[data-h-estado]')?.value||'BUENO',motivo=box.querySelector('[data-h-motivo]')?.value.trim(),file=box.querySelector('[data-h-foto]')?.files?.[0];if(!nombre)continue;const requiereSerie=ckNorm(box.querySelector('[data-h-serie]')?.dataset?.requiereSerie||'NO');if(requiereSerie==='SI'&&!serie)throw new Error('Debe ingresar el código o serie de '+nombre);if((estado==='REGULAR'||estado==='MALO')&&!motivo)throw new Error('Debe ingresar el motivo de '+nombre);if(estado==='MALO'&&!file)throw new Error('Debe subir la foto de '+nombre+' porque está en estado MALO');out.push({herramienta:nombre,codigoSerie:serie,estado,motivo,foto:file?await ckFileData(file):null});}return out;}
...
function ckDetallePorTipoV141(x){
  ...
  if(t==='HERRAMIENTAS'){
    ...
    return `<details class="ck-details"><summary>Ver herramientas y estados</summary><div style="margin-top:8px">${hs.length?hs.map(h=>`<div class="ck-detail-box" style="margin-bottom:7px"><b>${ckEsc(h.herramienta)}</b> ${ckEstado(h.estado)}${h.codigoSerie?`<div class="ck-meta">Serie: ${ckEsc(h.codigoSerie)}</div>`:''}${h.motivo?`<div class="ck-alerta-obs"><b>Motivo:</b> ${ckEsc(h.motivo)}</div>`:''}${h.foto?`<div class="ck-evidencias"><a href="${ckEsc(h.foto)}" target="_blank">Ver foto</a><a href="${ckEsc(ckUrlDescargaEvidencia(h.foto))}" target="_blank">Descargar</a></div>`:''}</div>`).join(''):'<div class="ck-meta">Sin detalle registrado.</div>'}</div></details>`;
  }
