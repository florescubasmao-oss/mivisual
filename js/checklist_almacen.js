...
async function ckCargarLista(){
  ...
  try{
    ...
    c.innerHTML=toolbar+`<div class="ck-kpis"><div class="ck-kpi"><b>${counts.total}</b>Total</div><div class="ck-kpi"><b>${counts.pend}</b>Pendientes</div><div class="ck-kpi"><b>${counts.vb}</b>Visto bueno</div><div class="ck-kpi"><b>${counts.ok}</b>Conformes</div></div>${arr.map(x=>ckCard(x,ckUser())).join('')||'<div class="ck-card">No hay registros.</div>'}`;
