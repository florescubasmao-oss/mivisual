...
function ckPeriodoActual(){const p=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Lima',year:'numeric',month:'2-digit'}).formatToParts(new Date());return `${p.find(x=>x.type==='year')?.value}-${p.find(x=>x.type==='month')?.value}`;}
function ckClavePeriodoRegistro(x){return ckFechaISO(x?.fechaGestion||x?.fechaRegistro).slice(0,7);}
function ckNombrePeriodo(clave){const m=String(clave||'').match(/^(\d{4})-(\d{2})$/);if(!m)return 'SIN PERIODO';return new Intl.DateTimeFormat('es-PE',{month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(Date.UTC(Number(m[1]),Number(m[2])-1,1))).toUpperCase();}
function ckPeriodosDisponibles(arr){return [...new Set([ckPeriodoActual(),...(arr||[]).map(ckClavePeriodoRegistro).filter(Boolean)])].sort().reverse();}
function ckSelectorPeriodoHtml(arr){return `<div class="ck-field"><label>Periodo</label><select id="ckFiltroPeriodo" onchange="ckCambiarPeriodo()">${ckPeriodosDisponibles(arr).map(p=>`<option value="${p}" ${p===CK_PERIODO_SELECCIONADO?'selected':''}>${ckNombrePeriodo(p)} — ${p===ckPeriodoActual()?'EN CURSO':'HISTÓRICO'}</option>`).join('')}</select></div>`;}
function ckFiltrarPeriodo(arr){return (arr||[]).filter(x=>ckClavePeriodoRegistro(x)===CK_PERIODO_SELECCIONADO);}
function ckCambiarPeriodo(){CK_PERIODO_SELECCIONADO=document.getElementById('ckFiltroPeriodo')?.value||ckPeriodoActual();if(ckUser().perfil==='TECNICO')ckRenderHistorialTecnico();else ckAplicarFiltrosVisuales();}
function ckRenderHistorialTecnico(){const box=document.getElementById('ckLista');if(!box)return;const arr=ckFiltrarPeriodo(CK_LISTA_ACTUAL);box.innerHTML=`<div class="ck-filter-panel"><div class="ck-filter-grid">${ckSelectorPeriodoHtml(CK_LISTA_ACTUAL)}</div></div><h3 class="ck-sec">Historial · ${ckNombrePeriodo(CK_PERIODO_SELECCIONADO)}</h3>${arr.map(x=>ckCard(x,ckUser())).join('')||'<div class="ck-card">No hay registros en este periodo.</div>'}`;}
...
const CK_HERRAMIENTAS_FALLBACK_V143=["FUSIONADORA","POWER METER","VFL","CLEAVER / CORTADORA","PELADORA DE FIBRA","TALADRO","ESCALERA","ALICATE","DESTORNILLADORES","WINCHA"];
let CK_HERRAMIENTAS_V140=CK_HERRAMIENTAS_FALLBACK_V143.map(h=>({herramienta:h,categoria:"",requiereSerie:"NO"}));
let CK_CATALOGO_HERRAMIENTAS_CARGADO_V143=false;
async function ckCargarCatalogoHerramientasV143(forzar=false){
  if(CK_CATALOGO_HERRAMIENTAS_CARGADO_V143&&!forzar)return CK_HERRAMIENTAS_V140;
  try{
    const u=ckUser();
    const d=await ckApi({accion:'obtenerCatalogoHerramientasChecklist',usuario:u.usuario});
    const lista=Array.isArray(d.herramientas)?d.herramientas:[];
    if(lista.length)CK_HERRAMIENTAS_V140=lista.map(x=>({herramienta:(x.herramienta||'').toString().trim(),categoria:(x.categoria||'').toString().trim(),requiereSerie:ckNorm(x.requiereSerie||'NO')})).filter(x=>x.herramienta);
    CK_CATALOGO_HERRAMIENTAS_CARGADO_V143=true;
  }catch(e){
    console.warn('No se pudo cargar CATALOGO_HERRAMIENTAS; se usa respaldo local.',e);
  }
  return CK_HERRAMIENTAS_V140;
}
...
async function ckCambioTipoChecklist(){const t=document.getElementById('ckTipoChecklist')?.value||'MATERIALES';const c=document.getElementById('ckFormularioTipo');if(t==='HERRAMIENTAS'){if(c)c.innerHTML='<div class="ck-card">Cargando catálogo de herramientas...</div>';await ckCargarCatalogoHerramientasV143(true);}if(c)c.innerHTML=ckFormularioPorTipo(t);if(t==='MATERIALES')Object.keys(CK_EQUIPOS).forEach(ckAddEquipo);}
function ckHerramientasHtml(){return `<div class="ck-card"><h3 class="ck-sec">🧰 Herramientas</h3><div class="act-note">Bueno: cantidad obligatoria, sin motivo ni foto. Regular: cantidad y motivo obligatorios. Malo: cantidad, motivo y foto obligatorios.</div><div id="ckHerramientasLista">${CK_HERRAMIENTAS_V140.map((item,i)=>{const h=typeof item==='string'?item:item.herramienta;const categoria=typeof item==='string'?'':(item.categoria||'');const requiereSerie=typeof item==='string'?'NO':ckNorm(item.requiereSerie||'NO');return `<div class="ck-herramienta-box" data-h-nombre="${ckEsc(h)}"><div class="ck-herramienta-titulo">${ckEsc(h)}</div>${categoria?`<div class="ck-herramienta-categoria">${ckEsc(categoria)}</div>`:''}<div class="ck-grid"><div class="ck-field"><label>Cantidad *</label><input data-h-cantidad type="number" min="1" step="1" value="" placeholder="Ingrese cantidad" oninput="ckActualizarTarjetaHerramienta(this)"></div>${requiereSerie==='SI'?`<div class="ck-field"><label>Código / Serie *</label><input data-h-serie data-requiere-serie="SI" placeholder="Obligatorio" oninput="ckActualizarTarjetaHerramienta(this)"></div>`:''}<div class="ck-field"><label>Estado</label><select data-h-estado onchange="ckEstadoHerramienta(this)"><option value="BUENO">BUENO</option><option value="REGULAR">REGULAR</option><option value="MALO">MALO</option></select></div><div class="ck-field" data-h-motivo-wrap style="display:none"><label>Motivo *</label><textarea data-h-motivo rows="2" oninput="ckActualizarTarjetaHerramienta(this)"></textarea></div><div class="ck-field" data-h-foto-wrap style="display:none"><label>Foto obligatoria por estado Malo *</label><input data-h-foto type="file" accept="image/*" capture="environment" onchange="ckActualizarTarjetaHerramienta(this)"></div></div></div>`;}).join('')}</div></div>`;}

function ckActualizarTarjetaHerramienta(origen){
  const box=origen&&origen.closest?origen.closest('.ck-herramienta-box'):null;
  if(!box)return;
  const cantidad=Number(box.querySelector('[data-h-cantidad]')?.value||0);
  const serieInput=box.querySelector('[data-h-serie]');
  const requiereSerie=!!serieInput;
  const serie=(serieInput?.value||'').trim();
  const estado=box.querySelector('[data-h-estado]')?.value||'BUENO';
  const motivo=(box.querySelector('[data-h-motivo]')?.value||'').trim();
  const foto=box.querySelector('[data-h-foto]')?.files?.length>0;
  let completo=cantidad>0;
  if(requiereSerie&&!serie)completo=false;
  if((estado==='REGULAR'||estado==='MALO')&&!motivo)completo=false;
  if(estado==='MALO'&&!foto)completo=false;
  box.classList.toggle('ck-progress-green',completo&&estado!=='MALO');
  box.classList.toggle('ck-progress-red',estado==='MALO');
}
function ckEstadoHerramienta(sel){
  const box=sel&&sel.closest?sel.closest('.ck-herramienta-box'):null;
  if(!box)return;
  const estado=sel.value||'BUENO';
  const motivoWrap=box.querySelector('[data-h-motivo-wrap]');
  const fotoWrap=box.querySelector('[data-h-foto-wrap]');
  const motivo=box.querySelector('[data-h-motivo]');
  const foto=box.querySelector('[data-h-foto]');
  if(motivoWrap)motivoWrap.style.display=(estado==='REGULAR'||estado==='MALO')?'block':'none';
  if(fotoWrap)fotoWrap.style.display=estado==='MALO'?'block':'none';
  if(estado==='BUENO'&&motivo)motivo.value='';
  if(estado!=='MALO'&&foto)foto.value='';
  ckActualizarTarjetaHerramienta(sel);
}
function ckFormularioPorTipo(t){
  const tipo=ckNorm(t||'MATERIALES');
  if(tipo==='MATERIALES')return `<h3 class="ck-sec">Equipos y evidencias</h3><div class="ck-grid ck-equipos-grid">${Object.keys(CK_EQUIPOS).map(ckEquipoBlock).join('')}</div><h3 class="ck-sec">Materiales</h3><div class="ck-grid">${ckField('ckCableDrop','Cable Drop/Bobina','metros')}${ckField('ckPre50','Preconectorizado 50 m','cantidad')}${ckField('ckPre100','Preconectorizado 100 m','cantidad')}${ckField('ckPre150','Preconectorizado 150 m','cantidad')}${ckField('ckPre200','Preconectorizado 200 m','cantidad')}${ckField('ckAnclaje','Anclaje P','cantidad')}${ckField('ckBand','Cinta Band-It','metros')}${ckField('ckHebilla','Hebilla 3/4','unidades')}${ckField('ckAcoplador','Acoplador','unidades')}${ckField('ckRoseta','Roseta','unidades')}${ckField('ckConectores','Conectores ópticos','unidades')}${ckField('ckTempladores','Templadores','unidades')}${ckField('ckSplitter','Splitter','unidades')}${ckField('ckClevis','Clevis','unidades')}${ckField('ckCat5','Cable UTP CAT5','unidades')}${ckField('ckCat6','Cable UTP CAT6','unidades')}${ckField('ckApc','Patchcord APC-APC','unidades')}${ckField('ckUpc','Patchcord UPC-APC','unidades')}${ckField('ckRj45','Conector RJ45','unidades')}</div>`;
  if(tipo==='HERRAMIENTAS')return ckHerramientasHtml();
  if(tipo==='UNIDAD VEHICULAR')return `<h3 class="ck-sec">🚐 Unidad Vehicular</h3><div class="ck-grid">${ckInputArchivo('ckUnidadFrente','Foto frontal')}${ckInputArchivo('ckUnidadPosterior','Foto posterior')}${ckInputArchivo('ckUnidadIzq','Foto lado izquierdo')}${ckInputArchivo('ckUnidadDer','Foto lado derecho')}${ckInputArchivo('ckExtintor','Foto del extintor')}${ckInputArchivo('ckBotiquin','Foto del botiquín')}${ckInputArchivo('ckReja','Foto de reja separadora')}${ckInputArchivo('ckParrilla1','Parrilla homologada - foto 1')}${ckInputArchivo('ckParrilla2','Parrilla homologada - foto 2')}<div class="ck-field"><label>Observación</label><textarea id="ckObsUnidad"></textarea></div></div>`;
  if(tipo==='DOCUMENTACION')return `<h3 class="ck-sec">📄 Documentación</h3><h4>Licencia de conducir</h4><div class="ck-grid"><div class="ck-field"><label>Fecha de vencimiento</label><input id="ckLicVence" type="date"></div>${ckInputArchivo('ckLicFrente','Licencia - frente')}${ckInputArchivo('ckLicReverso','Licencia - reverso')}</div><h4>SOAT</h4><div class="ck-grid"><div class="ck-field"><label>Fecha de vencimiento</label><input id="ckSoatVence" type="date"></div>${ckInputArchivo('ckSoatArchivo','SOAT - PDF o foto','image/*,application/pdf')}</div><h4>Revisión técnica</h4><div class="ck-grid"><div class="ck-field"><label>Fecha de vencimiento</label><input id="ckRevVence" type="date"></div>${ckInputArchivo('ckRevArchivo','Revisión técnica - foto o PDF','image/*,application/pdf')}<div class="ck-field"><label>Observación</label><textarea id="ckObsDoc"></textarea></div></div>`;
...
async function ckRecolectarHerramientas(){const out=[];for(const box of document.querySelectorAll('#ckHerramientasLista .ck-herramienta-box')){const nombre=(box.dataset.hNombre||'').trim(),cantidad=Number(box.querySelector('[data-h-cantidad]')?.value||0),serieInput=box.querySelector('[data-h-serie]'),serie=serieInput?.value.trim()||'',estado=box.querySelector('[data-h-estado]')?.value||'BUENO',motivo=box.querySelector('[data-h-motivo]')?.value.trim(),file=box.querySelector('[data-h-foto]')?.files?.[0];if(!nombre)continue;if(!Number.isFinite(cantidad)||cantidad<=0)throw new Error('Debe ingresar una cantidad mayor a cero para '+nombre);const requiereSerie=serieInput?ckNorm(serieInput.dataset?.requiereSerie||'NO'):'NO';if(requiereSerie==='SI'&&!serie)throw new Error('Debe ingresar el código o serie de '+nombre);if((estado==='REGULAR'||estado==='MALO')&&!motivo)throw new Error('Debe ingresar el motivo de '+nombre);if(estado==='MALO'&&!file)throw new Error('Debe subir la foto de '+nombre+' porque está en estado MALO');out.push({herramienta:nombre,cantidad,codigoSerie:serie,estado,motivo,foto:file?await ckFileData(file):null});}return out;}
...
/* =========================
   ...
   Filtros y acciones por función
========================= */
function ckTipoLabelV141(tipo){const t=ckNorm(tipo||'MATERIALES');return {'MATERIALES':'📦 Materiales','HERRAMIENTAS':'🧰 Herramientas','UNIDAD VEHICULAR':'🚐 Unidad Vehicular','DOCUMENTACION':'📄 Documentación','EPP':'🦺 EPP'}[t]||t;}
function ckResultadoTipoV141(x){const t=ckNorm(x.tipoChecklist);if(t==='HERRAMIENTAS')return x.resultadoHerramientas||'';if(t==='UNIDAD VEHICULAR')return x.resultadoUnidad||'';if(t==='DOCUMENTACION')return x.resultadoDocumentacion||'';if(t==='EPP')return x.resultadoEpp||'';return x.resultadoJefatura||x.resultadoAlmacen||'';}
function ckLinksV141(items){return items.filter(Boolean).map((u,i)=>`<a href="${ckEsc(u)}" target="_blank" rel="noopener noreferrer">Evidencia ${i+1}</a>`).join('');}
function ckDetallePorTipoV141(x){
  const t=ckNorm(x.tipoChecklist||'MATERIALES');
  if(t==='MATERIALES')return ckDetalle(x);
  if(t==='HERRAMIENTAS'){
    const hs=x.herramientasDetalle||[];
    return `<details class="ck-details"><summary>Ver herramientas y estados</summary><div style="margin-top:8px">${hs.length?hs.map(h=>`<div class="ck-detail-box" style="margin-bottom:7px"><b>${ckEsc(h.herramienta)}</b> ${ckEstado(h.estado)}${Number(h.cantidad||0)>0?`<div class="ck-meta">Cantidad: ${ckEsc(h.cantidad)}</div>`:''}${h.codigoSerie?`<div class="ck-meta">Serie: ${ckEsc(h.codigoSerie)}</div>`:''}${h.motivo?`<div class="ck-alerta-obs"><b>Motivo:</b> ${ckEsc(h.motivo)}</div>`:''}${h.foto?`<div class="ck-evidencias"><a href="${ckEsc(h.foto)}" target="_blank">Ver foto</a><a href="${ckEsc(ckUrlDescargaEvidencia(h.foto))}" target="_blank">Descargar</a></div>`:''}</div>`).join(''):'<div class="ck-meta">Sin detalle registrado.</div>'}</div></details>`;
  }
  if(t==='UNIDAD VEHICULAR')return `<details class="ck-details"><summary>Ver evidencias de la unidad</summary><div class="ck-evidencias">${ckLinksV141([x.fotoUnidadFrente,x.fotoUnidadPosterior,x.fotoUnidadLadoIzquierdo,x.fotoUnidadLadoDerecho,x.fotoExtintor,x.fotoBotiquin,x.fotoRejaSeparadora,x.fotoParrilla1,x.fotoParrilla2])}</div>${x.observacionUnidad?`<div class="ck-alerta-obs">${ckEsc(x.observacionUnidad)}</div>`:''}</details>`;
  if(t==='DOCUMENTACION')return `<details class="ck-details"><summary>Ver documentación y vencimientos</summary><div class="ck-grid" style="margin-top:8px"><div class="ck-detail-box"><b>Licencia</b><div class="ck-meta">Vence: ${ckEsc(x.licenciaFechaVencimiento||'')}</div><div class="ck-evidencias">${ckLinksV141([x.licenciaFotoFrente,x.licenciaFotoReverso])}</div></div><div class="ck-detail-box"><b>SOAT</b><div class="ck-meta">Vence: ${ckEsc(x.soatFechaVencimiento||'')}</div><div class="ck-evidencias">${ckLinksV141([x.soatArchivo])}</div></div><div class="ck-detail-box"><b>Revisión técnica</b><div class="ck-meta">Vence: ${ckEsc(x.revisionTecnicaFechaVencimiento||'')}</div><div class="ck-evidencias">${ckLinksV141([x.revisionTecnicaArchivo])}</div></div><div class="ck-detail-box"><b>Estado</b><div>${ckEsc(x.estadoVencimiento||'')}</div></div></div>${x.observacionDocumentacion?`<div class="ck-alerta-obs">${ckEsc(x.observacionDocumentacion)}</div>`:''}</details>`;
  return `<details class="ck-details"><summary>Ver evidencias de EPP</summary><div class="ck-evidencias">${ckLinksV141([x.fotoPersonalCompleto,x.fotoBotas,x.fotoFotocheck])}</div>${x.observacionEpp?`<div class="ck-alerta-obs">${ckEsc(x.observacionEpp)}</div>`:''}</details>`;
}
function ckAccionesV141(x,u){
  const t=ckNorm(x.tipoChecklist||'MATERIALES');
  const material=['MATERIALES','HERRAMIENTAS'].includes(t),operativo=['UNIDAD VEHICULAR','DOCUMENTACION','EPP'].includes(t);
  if(u.perfil==='ALMACEN'&&material)return `<button class="ck-btn green" onclick="ckValidar('${x.id}','VISTO BUENO')">Visto bueno</button><button class="ck-btn orange" onclick="ckValidar('${x.id}','OBSERVADO')">Observado</button>`;
  if(u.perfil==='SUPERVISOR'&&operativo)return `<button class="ck-btn green" onclick="ckValidar('${x.id}','CONFORME')">Conforme</button><button class="ck-btn orange" onclick="ckValidar('${x.id}','OBSERVADO')">Observado</button>`;
  if((u.perfil==='JEFATURA ALMACEN'||u.perfil==='JEFATURA DE ALMACEN')&&material)return `<button class="ck-btn green" onclick="ckValidar('${x.id}','CONFORME')">Conforme</button><button class="ck-btn orange" onclick="ckValidar('${x.id}','OBSERVADO')">Observado</button>`;
  if(['JEFATURA','ADMIN','ADMINISTRADOR'].includes(u.perfil)&&operativo)return `<button class="ck-btn green" onclick="ckValidar('${x.id}','CONFORME')">Conforme final</button><button class="ck-btn orange" onclick="ckValidar('${x.id}','OBSERVADO')">Observado</button>`;
  return '';
}
function ckCard(x,u){
  const estado=ckNorm(x.estadoGeneral),observado=estado.includes('OBSERVADO'),conforme=estado==='CONFORME'||estado.includes('CONFORME');
  const motivo=x.motivoJefatura||x.motivoAlmacen||'',quien=x.validadoJefaturaPor||x.validadoAlmacenPor||'';
...
function ckRenderFiltrosRolV141(arr,u){
  const sedes=[...new Set(arr.map(x=>ckNorm(x.sede)).filter(Boolean))].sort(),cuads=[...new Set(arr.map(x=>(x.cuadrilla||'').toString().trim()).filter(Boolean))].sort(),supervisores=[...new Set(arr.map(x=>(x.supervisor||'').toString().trim()).filter(Boolean))].sort(),estados=[...new Set(arr.map(x=>(x.estadoGeneral||'').toString().trim()).filter(Boolean))].sort();
  const tipos=ckTiposPermitidosPerfilV141(u.perfil);
  const sedeField=['JEFATURA','GERENCIA LIMA','ADMIN','ADMINISTRADOR','JEFATURA ALMACEN','JEFATURA DE ALMACEN'].includes(u.perfil)?`<div class="ck-field"><label>Sede</label><select id="ckFiltroSede" onchange="ckAplicarFiltrosVisuales()"><option value="">Todas</option>${sedes.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div>`:'';
  const supField=['JEFATURA','ADMIN','ADMINISTRADOR'].includes(u.perfil)?`<div class="ck-field"><label>Supervisor</label><select id="ckFiltroSupervisor" onchange="ckAplicarFiltrosVisuales()"><option value="">Todos</option>${supervisores.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div>`:'';
  const herramientaField=['ALMACEN','JEFATURA ALMACEN','JEFATURA DE ALMACEN','JEFATURA','GERENCIA LIMA','ADMIN','ADMINISTRADOR'].includes(u.perfil)?`<div class="ck-field"><label>Estado herramienta</label><select id="ckFiltroHerramienta" onchange="ckAplicarFiltrosVisuales()"><option value="">Todos</option><option>BUENO</option><option>REGULAR</option><option>MALO</option></select></div>`:'';
  return `<div class="ck-filter-panel"><div class="ck-filter-grid">${ckSelectorPeriodoHtml(arr)}${sedeField}<div class="ck-field"><label>Tipo</label><select id="ckFiltroTipo" onchange="ckAplicarFiltrosVisuales()"><option value="">Todos</option>${tipos.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div><div class="ck-field"><label>Cuadrilla</label><select id="ckFiltroCuadrilla" onchange="ckAplicarFiltrosVisuales()"><option value="">Todas</option>${cuads.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div>${supField}<div class="ck-field"><label>Fecha desde</label><input id="ckFiltroDesde" type="date" onchange="ckAplicarFiltrosVisuales()"></div><div class="ck-field"><label>Fecha hasta</label><input id="ckFiltroHasta" type="date" onchange="ckAplicarFiltrosVisuales()"></div><div class="ck-field"><label>Estado</label><select id="ckFiltroEstado" onchange="ckAplicarFiltrosVisuales()"><option value="">Todos</option>${estados.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div><div class="ck-field"><label>Vencimiento</label><select id="ckFiltroVencimiento" onchange="ckAplicarFiltrosVisuales()"><option value="">Todos</option><option>VIGENTE</option><option>PROXIMO A VENCER</option><option>VENCIDO</option></select></div>${herramientaField}</div><div class="ck-actions" style="margin-top:9px"><button class="ck-btn blue" onclick="ckAplicarFiltrosVisuales()">Aplicar filtros</button><button class="ck-btn gray" onclick="ckLimpiarFiltrosVisuales()">Limpiar filtros</button></div></div>`;
}
function ckFiltrarVisualChecklist(arr){
  const sede=ckNorm(document.getElementById('ckFiltroSede')?.value),tipo=ckNorm(document.getElementById('ckFiltroTipo')?.value),cuad=(document.getElementById('ckFiltroCuadrilla')?.value||'').trim(),sup=ckNorm(document.getElementById('ckFiltroSupervisor')?.value),desde=document.getElementById('ckFiltroDesde')?.value||'',hasta=document.getElementById('ckFiltroHasta')?.value||'',estado=ckNorm(document.getElementById('ckFiltroEstado')?.value),vence=ckNorm(document.getElementById('ckFiltroVencimiento')?.value),herr=ckNorm(document.getElementById('ckFiltroHerramienta')?.value);
  return arr.filter(x=>{if(ckClavePeriodoRegistro(x)!==CK_PERIODO_SELECCIONADO)return false;if(sede&&ckNorm(x.sede)!==sede)return false;if(tipo&&ckNorm(x.tipoChecklist)!==tipo)return false;if(cuad&&(x.cuadrilla||'').toString().trim()!==cuad)return false;if(sup&&ckNorm(x.supervisor)!==sup)return false;const f=ckFechaISO(x.fechaGestion||x.fechaRegistro);if(desde&&f<desde)return false;if(hasta&&f>hasta)return false;if(estado&&ckNorm(x.estadoGeneral)!==estado)return false;if(vence&&ckNorm(x.estadoVencimiento)!==vence)return false;if(herr&&!(x.herramientasDetalle||[]).some(h=>ckNorm(h.estado)===herr))return false;return true;});
}
function ckConteosLista(arr){return {total:arr.length,pend:arr.filter(x=>ckNorm(x.estadoGeneral).startsWith('PENDIENTE')).length,vb:arr.filter(x=>ckNorm(x.estadoGeneral).includes('VISTO BUENO')||ckNorm(x.estadoGeneral).includes('CONFORME SUPERVISOR')).length,ok:arr.filter(x=>ckNorm(x.estadoGeneral)==='CONFORME').length,obs:arr.filter(x=>ckNorm(x.estadoGeneral).includes('OBSERVADO')).length,venc:arr.filter(x=>ckNorm(x.estadoVencimiento)==='VENCIDO'||ckNorm(x.estadoVencimiento)==='PROXIMO A VENCER').length};}
