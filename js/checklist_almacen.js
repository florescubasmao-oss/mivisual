...
function ckAddEquipo(key){
  ...
  row.innerHTML=`<div class="ck-equipo-num">${cfg.label} ${count+1}</div><div class="ck-equipo-inputs"><input type="text" placeholder="${cfg.serie}" data-ck-serie="${key}" oninput="ckAutoEquipo('${key}',this);ckActualizarEstadoEquipoFila(this)"><input type="file" accept="image/*" capture="environment" data-ck-foto="${key}" onchange="ckAutoEquipo('${key}',this);ckActualizarEstadoEquipoFila(this)"></div><div class="ck-progress-note">Pendiente: registre serie y fotografía</div>${count?`<button type="button" class="ck-equipo-remove" onclick="ckRemoveEquipo(this,'${key}')">×</button>`:''}`;
...
function ckCard(x,u){
  ...
  const alerta=observado?`<div class="ck-alerta-obs"><b>Motivo de observación:</b> ${ckEsc(motivo||'Sin motivo registrado')}${quien?`<br><b>Observado por:</b> ${ckEsc(quien)}`:''}${fechaObs?` · ${ckEsc(fechaObs)} ${ckEsc(horaObs)}`:''}</div>`:'';
...
async function ckValidar(id,resultado){
  if(resultado==='CONFORME'){
    ckMostrarModal(`<button class="ck-modal-close" onclick="ckCerrarModal()">×</button><h3>Confirmar finalización</h3><p>¿Confirma que el checklist está correcto?</p><p class="ck-meta">Esta acción dejará el checklist como <b>CONFORME</b> y finalizará el proceso.</p><div class="ck-actions"><button class="ck-btn green" onclick="ckConfirmarConforme('${ckEsc(id)}')">Confirmar y finalizar</button><button class="ck-btn gray" onclick="ckCerrarModal()">Cancelar</button></div>`);return;
  }
  let motivo='';if(resultado==='OBSERVADO'){motivo=prompt('Ingrese el motivo de observación:')||'';if(!motivo)return;}
  await ckEnviarValidacion(id,resultado,motivo);
}
