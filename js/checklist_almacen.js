...
function ckRenderFiltrosRolV141(arr,u){
  ...
  const sedeField=['JEFATURA','ADMIN','ADMINISTRADOR','JEFATURA ALMACEN','JEFATURA DE ALMACEN'].includes(u.perfil)?`<div class="ck-field"><label>Sede</label><select id="ckFiltroSede" onchange="ckAplicarFiltrosVisuales()"><option value="">Todas</option>${sedes.map(v=>`<option>${ckEsc(v)}</option>`).join('')}</select></div>`:'';
