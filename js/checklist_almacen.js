// MI VISUAL - Checklist Almacén V95
...
function ckEquipoBlock(key){
  ...
  return `<div class="ck-photo-box ck-equipo-box"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><b style="font-size:13px">${cfg.label}</b><small id="ckCount_${key}">0/${cfg.max}</small></div><div id="ckEquipos_${key}"></div><button type="button" class="ck-btn gray" onclick="ckAddEquipo('${key}')">+ Agregar ${cfg.label}</button></div>`;
}
...
async function mostrarChecklistAlmacen(){const u=ckUser();mostrarPantalla(ckStyle()+`<div class="ck-wrap"><div class="ck-head"><h2>✅ CHECKLIST ALMACÉN</h2><p>Control de equipos, materiales y evidencias por cuadrilla.</p></div><div id="ckContenido"><div class="ck-card">Cargando...</div></div></div>`);if(u.perfil==='TECNICO')ckRenderForm();else await ckCargarLista();}
function ckRenderForm(){const u=ckUser(),c=document.getElementById('ckContenido');c.innerHTML=`<div class="ck-card"><h3 style="margin-top:0">Nuevo checklist</h3><div class="ck-grid"><div class="ck-field"><label>Nombres y apellidos</label><input id="ckNombres" value="${ckEsc(u.nombres)}"></div><div class="ck-field"><label>Fecha de gestión</label><input id="ckFecha" type="date" value="${new Date().toISOString().slice(0,10)}"></div><div class="ck-field"><label>Sede</label><input value="${ckEsc(u.sede)}" disabled></div><div class="ck-field"><label>Cuadrilla</label><input value="${ckEsc(u.cuadrilla)}" disabled></div></div>
...
async function ckCargarLista(){const c=document.getElementById('ckContenido')||document.getElementById('ckLista');if(!c)return;try{const d=await ckApi({accion:'listarChecklistAlmacen',usuario:ckUser().usuario});const arr=d.checklist||[];const counts={total:arr.length,pend:arr.filter(x=>ckNorm(x.estadoGeneral)==='PENDIENTE').length,vb:arr.filter(x=>ckNorm(x.estadoGeneral).includes('VISTO BUENO')).length,ok:arr.filter(x=>ckNorm(x.estadoGeneral)==='CONFORME').length};c.innerHTML=`<div class="ck-kpis"><div class="ck-kpi"><b>${counts.total}</b>Total</div><div class="ck-kpi"><b>${counts.pend}</b>Pendientes</div><div class="ck-kpi"><b>${counts.vb}</b>Visto bueno</div><div class="ck-kpi"><b>${counts.ok}</b>Conformes</div></div>${arr.map(x=>ckCard(x,ckUser())).join('')||'<div class="ck-card">No hay registros.</div>'}`;}catch(e){c.innerHTML=`<div class="ck-card">${ckEsc(e.message)}</div>`}}
