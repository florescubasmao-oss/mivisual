...
(function(){
  ...
  function renderDetalle(filtro){
    ...
    document.getElementById("pw487Detalle").innerHTML=`<div style="margin:7px 0;font-size:12px"><b>${filas.length}</b> orden(es)</div><div class="pw487-wrap"><table class="pw487-table"><thead><tr><th>Sede</th><th>Cuadrilla WIN</th><th>Fecha</th><th>OrdenId</th><th>Código pedido</th><th>DNI</th><th>TipoTraba</th><th>Motivo finalización</th><th>Clasificación</th><th>Tipo de Partida</th><th>Puntos</th><th>Cuadrilla Partner</th><th>Intervención</th></tr></thead><tbody>${filas.map(function(x){return `<tr class="${x.requiereIntervencion?"pw487-duda":""}"><td>${esc(x.sede)}</td><td>${esc(x.cuadrillaWin)}</td><td>${esc(x.fecha)}</td><td><b>${esc(x.ordenId)}</b></td><td>${esc(x.codigoPedido)}</td><td>${esc(x.dni)}</td><td>${esc(x.tipoTrabajoWin)}</td><td>${esc(x.motivoFinalizacionWin)}</td><td>${esc(x.clasificacion)}</td><td>${esc(x.tipoPartida||"PENDIENTE")}</td><td>${puntos(x.puntos)}</td><td>${esc(x.cuadrillaPartner||"-")}</td><td>${esc(x.motivoIntervencion||"NO")}</td></tr>`;}).join("")}</tbody></table></div>`;
  }
  function render(r){
