/* MI VISUAL V487.2 - Vista paralela de Produccion WIN. No realiza escrituras. */
(function(){
  "use strict";
  ...
  function cargarGateVtrGar(){
    if(typeof window.mv4872AplicarReglaVtrGar === "function") return Promise.resolve();
    if(MV487_GATE_PROMESA) return MV487_GATE_PROMESA;
    MV487_GATE_PROMESA=new Promise(function(resolve,reject){
      const s=document.createElement("script");
      s.src="./js/produccion_vtr_gar_gate_v4872.js?v=V4872-SIN-REPORTE-CERO-PUNTOS";
      s.async=true;
      s.onload=function(){
        if(typeof window.mv4872AplicarReglaVtrGar !== "function") return reject(new Error("No se pudo activar la regla VTR/GAR V487.2."));
        resolve();
      };
      s.onerror=function(){MV487_GATE_PROMESA=null;reject(new Error("No se pudo cargar la regla VTR/GAR V487.2."));};
  ...
  function renderDetalle(filtro){
    ...
    const filas=(r.detalle||[]).filter(function(x){
      ...
      if(filtro==="SIN_REPORTE")return x.esVtrGar && !x.reportadaVtrGar;
      if(filtro==="NO_CONSIDERADA")return x.esVtrGar && x.estadoConsideracion==="NO CONSIDERADA";
      return true;
    });
    document.getElementById("pw487Detalle").innerHTML=`<div style="margin:7px 0;font-size:12px"><b>${filas.length}</b> orden(es). <b>La orden nunca desaparece:</b> si es VTR/GAR sin reporte se muestra como NO CONSIDERADA y con 0 puntos.</div><div class="pw487-wrap"><table class="pw487-table"><thead><tr><th>Sede</th><th>Cuadrilla WIN</th><th>Fecha</th><th>OrdenId</th><th>Código pedido</th><th>DNI</th><th>TipoTraba</th><th>Motivo finalización</th><th>VTR/GAR</th><th>Reporte / validación</th><th>Estado producción</th><th>Clasificación</th><th>Regla</th><th>Tipo de Partida</th><th>Puntos</th><th>Partida Partner</th><th>Cuadrilla Partner</th><th>Intervención</th></tr></thead><tbody>${filas.map(function(x){
      const noConsiderada=x.esVtrGar&&x.estadoConsideracion==="NO CONSIDERADA";
      const clase=noConsiderada?"pw487-no-considerada":(x.requiereIntervencion?"pw487-duda":"");
      const vtr=x.esVtrGar?x.tipoVtrGar:"-";
      const reporte=x.esVtrGar?(x.reportadaVtrGar?`${x.resultadoReporteVtrGar||"PENDIENTE"}${x.ticketReporteVtrGar?" · "+x.ticketReporteVtrGar:""}`:"SIN REPORTE"):"-";
      const estado=x.esVtrGar?`${x.estadoConsideracion} · ${x.detalleConsideracion}`:"CONSIDERADA · REGLA NORMAL";
      return `<tr class="${clase}"><td>${esc(x.sede)}</td><td>${esc(x.cuadrillaWin)}</td><td>${esc(x.fecha)}</td><td><b>${esc(x.ordenId)}</b></td><td>${esc(x.codigoPedido)}</td><td>${esc(x.dni)}</td><td>${esc(x.tipoTrabajoWin)}</td><td>${esc(x.motivoFinalizacionWin)}</td><td>${esc(vtr)}</td><td class="${x.esVtrGar&&!x.reportadaVtrGar?"pw487-no":""}">${esc(reporte)}</td><td class="${noConsiderada?"pw487-no":"pw487-ok"}">${esc(estado)}</td><td>${esc(x.clasificacion)}</td><td>${esc(x.regla)}</td><td>${esc(x.tipoPartida||"PENDIENTE")}</td><td>${puntos(x.puntos)}</td><td>${esc(x.tipoPartidaPartner||"-")}</td><td>${esc(x.cuadrillaPartner||"-")}</td><td>${esc(x.motivoConsideracion||x.motivoIntervencion||"NO")}</td></tr>`;
  ...
  function render(r){
    ...
    document.getElementById("pw487Estado").innerHTML=`<div class="pw487-alert"><b>Modo paralelo V487.2 · solo lectura</b><br>Calculado ${esc(r.calculadoAl)}. No se modificó Producción oficial ni ningún otro indicador.</div><div class="pw487-rule"><b>Regla VTR/GAR:</b> el trabajo FINALIZADO de WIN siempre aparece en el historial diario. Solo suma puntos cuando existe reporte en Validación Técnica y el resultado final es BONO. <b>Sin reporte = NO CONSIDERADA · 0 puntos.</b></div>`;
    document.getElementById("pw487Kpis").innerHTML=[
      ...
      kpi("VTR/GAR WIN",x.vtrGarDetectadas||0),kpi("VTR/GAR SIN REPORTE",x.vtrGarSinReporte||0),kpi("VTR/GAR HABILITADAS",x.vtrGarHabilitadas||0),
      kpi("VTR/GAR NO CONSID.",x.vtrGarNoConsideradas||0),kpi("PUNTOS ACTUALES",puntos(x.puntosActuales)),kpi("PUNTOS NUEVOS",puntos(x.puntosNuevos)),
      kpi("DIFERENCIA PUNTOS",puntos(x.diferenciaPuntos)),kpi("ÓRDENES ACTUALES",x.ordenesActuales),kpi("DIFERENCIA ÓRDENES",x.diferenciaOrdenes),kpi("REGLAS EXACTAS",(r.reglas||{}).exactas||0)
    ].join("");
    document.getElementById("pw487Meta").textContent=`Entrenamiento: ${r.periodoEntrenamiento} · ${r.reglas.filasEntrenamiento} coincidencias válidas · Catálogo vigente sin cambios · Regla VTR/GAR V487.2 aplicada en lectura.`;
    renderDetalle("TODAS");
  }
  ...
  async function consultar(){
    ...
    try{
      btn.disabled=true;
      estado.innerHTML="Calculando Producción WIN paralela y validando reportes VTR/GAR...";
      const base=await api({accion:"previsualizarProduccionWinParalelaV487",usuario:usuario(),periodo:periodo});
      await cargarGateVtrGar();
      const ajustado=await window.mv4872AplicarReglaVtrGar(base,base.periodo);
      render(ajustado);
    }catch(e){
      estado.innerHTML=`<div style="background:#7f1d1d;padding:11px;border-radius:10px"><b>No se mostró una simulación incompleta.</b><br>${esc(e.message)}</div>`;
    }finally{btn.disabled=false;}
  }

  window.mostrarProduccionWinParalelaV487=function(){
    const baseCss=typeof window.boCss==="function"?window.boCss():"";
    mostrarPantalla(baseCss+css()+`<div class="pw487"><h2>🧪 Producción WIN paralela · V487.2</h2><p>WIN define las órdenes FINALIZADAS. Partner apoya la clasificación. Validación Técnica controla si una VTR/GAR puede sumar puntos. Esta pantalla no guarda cambios.</p><div class="pw487-card"><div class="pw487-actions"><label><b>Período</b><br><select id="pw487Periodo" class="pw487-select"><option value="">Último disponible</option></select></label><button id="pw487Consultar" class="pw487-btn" onclick="mv487Consultar()">Calcular simulación</button><button class="pw487-btn alt" onclick="mostrarAdministracion()">⬅️ Volver</button></div><div id="pw487Estado" style="margin-top:12px">Listo para consultar MAPA_ORDENES, Validación Técnica y comparar con la Producción actual.</div></div><div id="pw487Kpis" class="pw487-grid"></div><div id="pw487Meta" style="font-size:11px;color:#cbd5e1;margin:10px 0"></div><div class="pw487-tabs">${[["TODAS","Todas"],["SIN_REPORTE","VTR/GAR sin reporte"],["NO_CONSIDERADA","No consideradas"],["AUTOMATICA","Automáticas"],["PARTNER","Usando Partner"],["DUDOSA","Dudosas"],["CUADRILLA","Cuadrilla diferente"],["INTERVENCION","Intervención"],["COMPARACION","Diferencia diaria"]].map(function(t){return `<button class="pw487-tab ${t[0]==="TODAS"?"act":""}" data-filtro="${t[0]}" onclick="mv487Filtrar('${t[0]}')">${t[1]}</button>`;}).join("")}</div><div id="pw487Detalle" class="pw487-card">Ejecute la simulación para ver el detalle por sede, cuadrilla, día y orden.</div></div>`);
  };
