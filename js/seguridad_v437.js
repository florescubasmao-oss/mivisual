/* ============================================================
   MI VISUAL V437 - SEGURIDAD ATS / PETAR DIGITAL
...
const LOGO_EMPRESA='./img/logo-visual-connections.png?v=V437';
...
function cacheKey(){return 'MI_VISUAL_SEG437_CTX_'+N(user())}
function cacheLeer(){
  try{
...
function renderAts(){
  ...
  h+=`<div class="seg-card"><b>Estado: ${e(a.estado)}</b>
    ...
    ${perfil()==='SUPERVISOR'&&['PENDIENTE SUPERVISOR','PENDIENTE ACEPTACION'].includes(N(a.estado))&&acept.length>=1?`<div class="seg-note seg-warn" style="margin-top:8px">Aceptación técnica: ${acept.length}/${(a.integrantes||[]).length}. Al autorizar, MI VISUAL completará cualquier firma faltante dejando trazabilidad del responsable.</div><div class="seg-actions"><button class="seg-btn green" onclick="seg433Revisar('AUTORIZAR')">Autorizar y completar</button><button class="seg-btn orange" onclick="seg433Revisar('OBSERVAR')">Observar</button><button class="seg-btn red" onclick="seg433Revisar('RECHAZAR')">Rechazar</button></div>`:''}
...
async function revisar(accion){
  ...
  try{await conProceso(msg,async()=>{procesoTexto('Obteniendo ubicación...');const g=await geo();procesoTexto(msg);await api({accion:'revisarAtsSupervisorV432',id:SEG.ats.id,resultado:accion,motivo,gps:g});cacheBorrar();procesoTexto('Actualizando documento...');await abrirAtsCore(SEG.ats.id)})}
  catch(err){alert(err.message)}
}
async function validarFinal(resultado){
  ...
  try{
    ...
    if(pdf)alert('Documento cerrado. PDF final disponible para descarga.');
  }catch(err){alert(err.message)}
}
