/* ============================================================
   MI VISUAL V438 - SEGURIDAD ATS / PETAR DIGITAL
...
const LOGO_EMPRESA='./img/logo-visual-connections.png?v=V438';
...
function cacheKey(){return 'MI_VISUAL_SEG438_CTX_'+N(user())}
function cacheLeer(){
  try{
...
async function revisar(accion){
  ...
  const msg=accion==='AUTORIZAR'?'Autorizando y generando PDF final...':accion==='OBSERVAR'?'Registrando observación...':'Rechazando ATS / PETAR...';
  try{
...
async function validarFinal(resultado){
  ...
  const msg=resultado==='VALIDAR'?'Validando y generando PDF final...':resultado==='OBSERVAR'?'Registrando observación...':'Rechazando ATS / PETAR...';
  try{
    let pdf=false;
    await conProceso(msg,async()=>{
