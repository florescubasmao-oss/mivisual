// MI VISUAL - Gestión de Actas v61

const API_ACTAS = "https://script.google.com/macros/s/AKfycbxirv6JT0W68dY0SZV7_mHzR8lzizybcDTyXTqvn8BOWMWEmYaJRx01DEpgTDBvbg_x/exec";
...
function estiloActas(){
    return `<style id="styleActas">
        ...
        .actas-head p{margin:0;opacity:.9;font-size:13px;line-height:1.4;}
        .actas-actions{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0;}
        .actas-btn{border:0;border-radius:12px;padding:10px 13px;font-weight:800;cursor:pointer;box-shadow:0 4px 12px rgba(15,23,42,.12);}
        ...
        .actas-msg{padding:10px;border-radius:12px;margin:10px 0;font-weight:700;}
...
function mostrarGestionActas(){
    ...
    if(u.perfil === "TECNICO") subtitulo = "Sube tus actas en PDF y consulta si están pendientes, observadas o finalizadas.";
    if(u.perfil === "ALMACEN") subtitulo = "Consulta de actas de tu sede. No tienes permiso para validar.";
    if(u.perfil === "SUPERVISOR") subtitulo = "Consulta de actas de tu sede. No tienes permiso para validar.";
    if(esJefaturaActas(u.perfil)) subtitulo = "Vista de resumen. Solo lectura.";
    if(esJefaturaAlmacenActas(u.perfil)) subtitulo = "Validación final de actas: correcto u observado.";
...
async function cargarActas(){
    ...
    try{
        ...
        if(!data.actas || data.actas.length === 0){
            lista.innerHTML = `<div class="actas-card">No hay actas registradas.</div>`;
            return;
        }
        lista.innerHTML = `
            <table class="actas-table">
                <thead><tr><th>Pedido</th><th>Cuadrilla</th><th>Sede</th><th>Tipo</th><th>Estado</th><th>PDF</th><th>Acción</th></tr></thead>
...
function badgeActa(a){
    ...
    const resultado = normalizarActas(a.resultadoValidacion);
    if(estado === "FINALIZADO") return `<span class="actas-badge actas-fin">FINALIZADO</span>`;
    if(resultado === "OBSERVADO") return `<span class="actas-badge actas-obs">OBSERVADO</span>`;
...
function botonesActa(a){
    ...
    if(esJefaturaAlmacenActas(u.perfil) && normalizarActas(a.estado) !== "FINALIZADO"){
    ...
    if(u.perfil === "TECNICO" && normalizarActas(a.resultadoValidacion) === "OBSERVADO"){
        html += ` <button class="actas-btn danger" onclick="mostrarFormularioActa('${a.codigoPedido || ""}')">Subir corrección</button>`;
    }
    return html;
}
...
function filaActaHtml(a){
    return `<tr>
        <td><b>${a.codigoPedido || "-"}</b><br><small>${a.codigoOrden || ""}</small></td>
        <td>${a.cuadrilla || "-"}</td>
        <td>${a.sede || "-"}</td>
        <td>${a.tipoPartida || "-"}</td>
        <td>${badgeActa(a)}${a.motivoObservacion ? `<br><small>${a.motivoObservacion}</small>` : ""}</td>
...
function cardActaHtml(a){
    return `<div class="actas-card">
        <b>${a.codigoPedido || "Acta"}</b> ${badgeActa(a)}<br>
        <small>${a.sede || ""} • ${a.cuadrilla || ""} • ${a.tipoPartida || ""}</small><br>
        ${a.motivoObservacion ? `<div class="actas-msg err">${a.motivoObservacion}</div>` : ""}
        <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
            ${a.linkActa ? `<a class="actas-btn sec" href="${a.linkActa}" target="_blank">Abrir PDF</a>` : ""}
            ${botonesActa(a)}
        </div>
    </div>`;
}
...
async function mostrarFormularioActa(codigoPedidoPrefill){
    ...
    mostrarPantalla(`
        ${estiloActas()}
        <div class="actas-wrap">
            <div class="actas-head"><h2>📄 Subir Acta Escaneada</h2><p>Solo se permite archivo PDF. Si el acta fue observada, sube aquí la corrección.</p></div>
            <form id="formActa" onsubmit="event.preventDefault(); guardarActa(this.querySelector('[data-guardar]'))">
                <div class="actas-grid">
                    <div class="actas-field"><label>Sede</label><input value="${u.sede}" disabled></div>
                    <div class="actas-field"><label>Cuadrilla</label><input value="${u.cuadrilla}" disabled></div>
                    <div class="actas-field"><label>Fecha de gestión</label><input type="date" id="actaFechaGestion" required></div>
                    <div class="actas-field"><label>Tipo de partida</label><select id="actaTipoPartida" required><option value="">Cargando...</option></select></div>
                    <div class="actas-field"><label>Código de orden</label><input id="actaCodigoOrden" required></div>
                    <div class="actas-field"><label>Código de pedido</label><input id="actaCodigoPedido" value="${codigoPedidoPrefill || ""}" required></div>
                ...
                <div id="actaMsg"></div>
                <div class="actas-actions">
                    <button class="actas-btn ok" data-guardar type="submit">Guardar Acta</button>
...
async function cargarTiposPartidaActas(){
    ...
    if(!sel) return;
    try{
        const data = await apiActas({accion:"listarTiposPartidaActas"});
        sel.innerHTML = `<option value="">Seleccione...</option>` + (data.tipos || []).map(t => `<option value="${t}">${t}</option>`).join("");
    }catch(err){
...
async function guardarActa(btn){
    ...
    try{
        ...
        if(msg) msg.innerHTML = `<div class="actas-msg ok">✅ Acta registrada correctamente.<br>Archivo: ${data.nombreArchivo}<br>Estado: PENDIENTE</div>`;
...
async function validarActa(id, resultado){
    ...
    if(resultado === "OBSERVADO"){
        ...
        if(!confirm("¿Confirmar acta correcta?")) return;
    }
    try{
        const u = usuarioActualActas();
