...
function pdRenderGestion(){
  ...
  <div class="pd-card"><div class="pd-toolbar">
    ...
    ${esSupervisor?`<div class="pd-field"><label>Sede</label><select id="pdSede" disabled><option>${pdEsc(u.sede)}</option></select></div>`:`<div class="pd-field"><label>Sede</label><select id="pdSede">${sedes.map(x=>`<option ${x===sedeSel?'selected':''}>${x}</option>`).join('')}</select></div>`}
