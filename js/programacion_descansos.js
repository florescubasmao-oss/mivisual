...
function pdEsPendienteEstado(estado){return pdNorm(estado).startsWith("PENDIENTE");}
function pdEstadoAnterior(item){
  const s=(item?.solicitudCambio||"").toString();
  if(s.indexOf("ESTADO_ANTERIOR:")===0) return pdNorm(s.replace("ESTADO_ANTERIOR:",""))||"EN CAMPO";
  return "EN CAMPO";
}
function pdEstadoVisible(item){
  if(!item)return "EN CAMPO";
  const ep=pdNorm(item.estadoProgramacion);
  if(ep==="APROBADO") return pdNorm(item.estadoDia||"EN CAMPO");
  if(ep==="RECHAZADO") return pdEstadoAnterior(item);
  return "EN CAMPO";
}
...
function pdRenderGestion(){
  ...
  const pendientes=PD_DATA.programacion
    .filter(x=>pdNorm(x.estadoProgramacion)===(esSupervisor?'PENDIENTE SUPERVISOR':'PENDIENTE JEFATURA'))
    .sort((a,b)=>(a.fecha||'').localeCompare(b.fecha||'')||pdNorm(a.cuadrilla).localeCompare(pdNorm(b.cuadrilla)));
  const alertasJefatura=esSupervisor?PD_DATA.programacion
    .filter(x=>pdNorm(x.estadoProgramacion)==='APROBADO'&&pdNorm(x.resultadoJefatura)==='APROBADO'&&pdNorm(x.validadoJefaturaPor)&&pdNorm(x.validadoJefaturaPor)!==pdNorm(u.usuario)&&pdNorm(x.sede)===pdNorm(u.sede))
    .sort((a,b)=>(b.fechaValidacionJefatura||b.fecha||'').toString().localeCompare((a.fechaValidacionJefatura||a.fecha||'').toString())):[];
  ...
  <div class="pd-card"><div class="pd-toolbar">
    ...
    ${esSupervisor?'<button class="pd-btn pd-orange" onclick="pdGuardarCambios()">Enviar programación / cambio a Jefatura</button>':'<button class="pd-btn pd-green" onclick="pdGuardarCambios()">Guardar programación</button><button class="pd-btn pd-blue" onclick="pdAprobarProgramacion()">Revisar y aprobar pendientes</button>'}
  </div></div>
  <div id="pdCobertura" class="pd-card">Calculando capacidad operativa...</div>
  <div id="pdConsultaDia" class="pd-card"></div>
  <div class="pd-card"><div class="pd-note">Vista operativa: <b>C = En campo</b>, <b>D = Descanso</b>. El mes principal siempre se visualiza completo y, según la fecha actual, incluye días del mes anterior o siguiente.</div><div id="pdCalendario" style="margin-top:8px"></div></div>
  <div class="pd-card"><b>${esSupervisor?'Solicitudes de técnicos pendientes de Supervisor':'Programaciones, cambios y alertas pendientes de Jefatura'}</b><div class="pd-list" style="margin-top:8px">${pendientes.length?pendientes.map(pdSolicitudCard).join(''):'<div class="pd-note">No hay solicitudes pendientes.</div>'}</div></div>
  ${esSupervisor?`<div class="pd-card"><b>Alertas de cambios aplicados por Jefatura</b><div class="pd-list" style="margin-top:8px">${alertasJefatura.length?alertasJefatura.map(x=>`<div class="pd-alert-row"><strong>${pdEsc(x.cuadrilla)} · ${pdEsc(x.fecha)}</strong><div>Cambio aplicado: ${pdEsc(x.estadoDia)}</div><div class="pd-note" style="color:inherit">Motivo: ${pdEsc(x.motivoJefatura||x.motivoSolicitud||'Sin comentario')}</div><div class="pd-note" style="color:inherit">Realizado por: ${pdEsc(x.validadoJefaturaPor||x.solicitadoPor)}</div></div>`).join(''):'<div class="pd-note">No hay cambios recientes realizados por Jefatura.</div>'}</div></div>`:''}`;
  pdRenderCalendario();pdCargarCobertura();pdRenderConsultaOperativa();
}
...
function pdRenderCalendario(){
  ...
  ordenadas.forEach(c=>{
    ...
    fechas.forEach(fecha=>{const key=c.cuadrilla+'|'+fecha,item=pdBuscar(c.cuadrilla,fecha),pend=item&&pdEsPendienteEstado(item.estadoProgramacion),estado=PD_CAMBIOS[key]||(pend?pdNorm(item.estadoDia||'EN CAMPO'):pdEstadoVisible(item)),cl=estado==='DESCANSO'?'pd-descanso':'pd-campo',domingo=pdDiaCorto(fecha)==='D',hoy=fecha===pdHoy(),comentario=item?(item.motivoSolicitud||item.motivoSupervisor||item.motivoJefatura||'Sin comentario'):'',origen=item?(item.solicitadoPor||item.validadoSupervisorPor||item.validadoJefaturaPor||''):'',detalle=pend?`${item.estadoProgramacion} · ${comentario} · ${origen}`:(item?.estadoProgramacion||'APROBADO');html+=`<td class="${domingo?'pd-sunday ':''}${hoy?'pd-current-day':''}"><button class="pd-day ${cl} ${pend?'pd-pendiente':''} ${hoy?'pd-hoy':''}" onclick="pdToggleDia('${pdEsc(c.cuadrilla)}','${fecha}',this)" title="${pdEsc(detalle)}">${estado==='DESCANSO'?'D':'C'}</button></td>`;});
    html+='</tr>';
  });
...
async function pdGuardarCambios(){try{const u=pdUser(),registros=Object.entries(PD_CAMBIOS).map(([k,estadoDia])=>{const i=k.lastIndexOf('|');return {cuadrilla:k.slice(0,i),fecha:k.slice(i+1),estadoDia};});if(!registros.length)return alert('No hay cambios para guardar.');const motivo=(prompt('Motivo de la programación o modificación (obligatorio):')||'').trim();if(!motivo)return alert('Debe ingresar el motivo.');const r=await pdApi({accion:'guardarProgramacionDescansos',usuario:u.usuario,registros,motivo});alert(`${r.guardados} registros enviados. Estado: ${r.estado}${r.alertas?' · Alertas: '+r.alertas:''}`);const per=document.getElementById('pdPeriodo').value;pdCapturarFiltros();await pdCargar(per);pdRenderGestion();}catch(e){alert(e.message);}}
...
function pdSolicitudCard(x){const u=pdUser(),solicitud=!!x.solicitudCambio&&!x.solicitudCambio.toString().startsWith('ESTADO_ANTERIOR:'),esJefatura=u.perfil!=='SUPERVISOR',comentario=x.motivoSolicitud||x.motivoSupervisor||x.motivoJefatura||'',origen=x.solicitadoPor||x.validadoSupervisorPor||'';return `<div class="pd-item"><strong>${pdEsc(x.cuadrilla)} · ${pdEsc(x.sede)} · ${pdEsc(x.plataforma)}</strong><div>${solicitud?`Cambio solicitado: ${x.fecha} → ${pdEsc(x.solicitudCambio)}`:`Programación/cambio: ${x.fecha} · ${pdEsc(x.estadoDia)}`}</div><span class="pd-status pendiente">${pdEsc(x.estadoProgramacion)}</span>${origen?`<div class="pd-note">Origen: ${pdEsc(origen)}</div>`:''}${comentario?`<div class="pd-note">Comentario: ${pdEsc(comentario)}</div>`:''}<div class="pd-actions">${solicitud?`<button class="pd-btn pd-green" onclick="pdValidarSolicitud('${pdEsc(x.id)}','APROBADO')">Aprobar</button><button class="pd-btn pd-red" onclick="pdValidarSolicitud('${pdEsc(x.id)}','RECHAZADO')">Rechazar</button>`:(esJefatura?`<button class="pd-btn pd-green" onclick="pdResolverProgramacion('${pdEsc(x.id)}','APROBADO')">Aprobar</button><button class="pd-btn pd-red" onclick="pdResolverProgramacion('${pdEsc(x.id)}','RECHAZADO')">Rechazar</button>`:'')}</div></div>`;}
