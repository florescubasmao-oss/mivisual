...
function pdSeleccionarTodosPendientes(marcado){PD_PENDIENTES_SELECCIONADOS.clear();if(marcado)(PD_DATA.programacion||[]).filter(x=>pdEsPendiente(x)&&pdNorm(x.tipoRegistro)!=='SOLICITUD_TECNICO').forEach(x=>{if(x&&x.id)PD_PENDIENTES_SELECCIONADOS.add(x.id);});document.querySelectorAll('.pd-check-pendiente').forEach(ch=>ch.checked=marcado);}

function pdEsPerfilJefatura(perfil){return ["JEFATURA","JEFATURA GENERAL","ADMIN","ADMINISTRADOR"].includes(pdNorm(perfil));}
function pdEsRevisorDescansos(perfil){return pdNorm(perfil)==="SUPERVISOR"||pdEsPerfilJefatura(perfil);}
function pdQuitarNotificacionDescansosMenu(){
  const card=document.getElementById("cardProgramacionDescansos");
  if(card){card.classList.remove("pd-card-con-pendientes");const badge=card.querySelector("#pdBadgeDescansos");if(badge)badge.remove();}
  const aviso=document.getElementById("pdAvisoPendientesDescansos");if(aviso)aviso.remove();
}
function pdAplicarNotificacionDescansosMenu(cantidad){
  const total=Math.max(0,Number(cantidad)||0);
  const card=document.getElementById("cardProgramacionDescansos");
  if(!card)return;
  let badge=card.querySelector("#pdBadgeDescansos");
  if(!total){pdQuitarNotificacionDescansosMenu();return;}
  card.classList.add("pd-card-con-pendientes");
  if(!badge){badge=document.createElement("span");badge.id="pdBadgeDescansos";badge.className="pd-card-badge-descansos";card.appendChild(badge);}
  badge.textContent=total>99?"99+":String(total);
  badge.setAttribute("aria-label",`${total} solicitudes de descanso pendientes`);
  const welcome=document.getElementById("mv55Welcome");
  if(welcome){
    let aviso=document.getElementById("pdAvisoPendientesDescansos");
    if(!aviso){aviso=document.createElement("button");aviso.id="pdAvisoPendientesDescansos";aviso.type="button";aviso.className="pd-aviso-pendientes-descansos";aviso.onclick=mostrarProgramacionDescansos;welcome.appendChild(aviso);}
    aviso.innerHTML=`<span>🔔</span><strong>${total} solicitud${total===1?'':'es'} de descanso pendiente${total===1?'':'s'}</strong><small>Toca para revisar</small>`;
  }
}
async function pdActualizarNotificacionesDescansosMenu(){
  const u=pdUser();
  if(!pdEsRevisorDescansos(u.perfil)){pdQuitarNotificacionDescansosMenu();return 0;}
  try{
    const r=await pdApi({accion:"obtenerNotificacionesDescansos",usuario:u.usuario});
    pdAplicarNotificacionDescansosMenu(r.pendientes||0);
    return Number(r.pendientes)||0;
  }catch(e){console.warn("No se pudo consultar notificaciones de descansos",e);return 0;}
}
...
async function actualizarIndicadorDescansoMenu(){
  ...
  const muestraEstadoPersonal=["TECNICO","SUPERVISOR","ALMACEN"].includes(u.perfil);

  if(!muestraEstadoPersonal){
    ...
    try{
      ...
      indicador=document.getElementById("pdEstadoMenu");
      if(indicador){indicador.className="pd-menu-status campo";indicador.innerHTML='<span class="pd-menu-dot"></span><strong>EN CAMPO</strong><small>Ver programación</small>';}
    }
  }
...
async function pdAbrirModuloDescansos(vistaPersonal){
  ...
  try{await pdCargar();pdRender();await pdActualizarNotificacionesDescansosMenu();}catch(e){const c=document.getElementById("pdContenido")||document.getElementById("pantalla");if(c)c.innerHTML=`<div class="pd-wrap"><div class="pd-alert">${pdEsc(e.message)}</div></div>`;else alert(e.message);}
}
...
function pdEstadoVigenteTecnico(item){
  if(!item)return "EN CAMPO";
  if(pdEsPendiente(item))return pdNormalizarEstado(item.estadoDia||"EN CAMPO");
  return pdNormalizarEstado(pdEstadoVisible(item));
}
function pdCalendarioTecnicoHtml(cuadrilla,periodo){
  const [anio,mes]=periodo.split("-").map(Number),dias=pdDiasMes(periodo);
  const inicio=(new Date(anio,mes-1,1).getDay()+6)%7;
  let html='<div class="pd-tech-calendar">';
  ['LUN','MAR','MIÉ','JUE','VIE','SÁB','DOM'].forEach(d=>html+=`<div class="pd-tech-weekday">${d}</div>`);
...
async function pdCambiarMesTecnico(periodo){
  try{await pdCargar(periodo,true);pdRenderTecnico();}catch(e){alert(e.message);}
}
...
async function pdSolicitarCambio(){try{const u=pdUser();const fechaDescansoActual=document.getElementById('pdDescansoActual').value,nuevaFecha=document.getElementById('pdNuevaFecha').value,motivo=document.getElementById('pdMotivoSolicitud').value.trim();await pdApi({accion:'solicitarCambioDescanso',usuario:u.usuario,fechaDescansoActual,nuevaFecha,motivo});alert('Solicitud enviada al Supervisor.');pdLimpiarCachePeriodo(PD_DATA.periodo);await pdCargar(PD_DATA.periodo,true);pdRenderTecnico();await pdActualizarNotificacionesDescansosMenu();}catch(e){alert(e.message);}}
...
function pdRenderGestion(){
  ...
  const pendientes=soloLectura?[]:PD_DATA.programacion.filter(x=>{const e=pdNorm(x.estadoValidacion||x.estadoProgramacion).replace(/_/g,' ');return esSupervisor?e==='PENDIENTE SUPERVISOR':['PENDIENTE SUPERVISOR','PENDIENTE JEFATURA','OBSERVADO'].includes(e);});
  const pendientesGenericos=pendientes.filter(x=>pdNorm(x.tipoRegistro)!=='SOLICITUD_TECNICO');
  ...
  <div class="pd-card"><div class="pd-collapse-head"><div><span class="pd-collapse-title">${esSupervisor?'Solicitudes de técnicos pendientes de Supervisor':'Cambios pendientes de Jefatura'}</span><div class="pd-note">${pendientes.length?pendientes.length+' solicitud(es) pendiente(s).':'No hay solicitudes pendientes.'}</div></div><button type="button" class="pd-collapse-btn" data-pd-toggle="pdPendientesBody" onclick="pdTogglePanel('pdPendientesBody')">▼ Ver</button></div><div id="pdPendientesBody" class="pd-collapse-body">${(!esSupervisor&&pendientesGenericos.length)?`<div class="pd-actions" style="margin-bottom:8px"><label class="pd-select-all"><input type="checkbox" onchange="pdSeleccionarTodosPendientes(this.checked)"> Seleccionar cambios de programación</label><button class="pd-btn pd-green" onclick="pdResolverSeleccionados('APROBADO')">Aprobar seleccionados</button><button class="pd-btn pd-orange" onclick="pdResolverSeleccionados('OBSERVADO')">Observar seleccionados</button><button class="pd-btn pd-red" onclick="pdResolverSeleccionados('RECHAZADO')">Rechazar seleccionados</button></div>`:''}<div class="pd-list">${pendientes.length?pendientes.map(pdSolicitudCard).join(''):'<div class="pd-note">No hay solicitudes pendientes.</div>'}</div></div></div>
...
function pdToggleTecnicos(id,btn){const panel=document.getElementById(id);if(!panel)return;const abierto=panel.classList.toggle('abierto');if(btn)btn.textContent=(abierto?'▼':'▶')+' Técnicos ('+panel.children.length+')';}
...
function pdRenderCalendario(){
  ...
  ordenadas.forEach(c=>{
    ...
    const techId='pdTech_'+String(c.cuadrilla||'').replace(/[^a-zA-Z0-9]/g,'_');
    const integrantesHtml=tipo==='CUADRILLA'&&integrantes.length?`<button type="button" class="pd-tech-toggle" onclick="pdToggleTecnicos('${techId}',this)">▶ Técnicos (${integrantes.length})</button><div id="${techId}" class="pd-integrantes">${integrantes.map(n=>`<span>👤 ${pdEsc(n)}</span>`).join('')}</div>`:'';
...
async function pdResolverSeleccionados(resultado){
  ...
  try{
    ...
    await pdCargar(per,true);pdRenderGestion();await pdActualizarNotificacionesDescansosMenu();
  }catch(e){alert(e.message);}
}
function pdMostrarDetallePendiente(id){const x=PD_DATA.programacion.find(i=>i.id===id);const cont=document.getElementById('pdDetallePendiente');if(!x||!cont)return;const u=pdUser(),esSolicitud=pdNorm(x.tipoRegistro)==='SOLICITUD_TECNICO',anterior=pdNorm(x.estadoAnterior||x.estadoDia||'EN CAMPO'),propuesto=pdNorm(x.estadoNuevo||x.solicitudCambio||pdEstadoPropuesto(x));const cambioHtml=esSolicitud?`<div>Descanso actual: <b>${pdEsc(x.fecha)}</b></div><div>Nueva fecha solicitada: <b>${pdEsc(x.solicitudCambio)}</b></div>`:`<div>Estado vigente: <b>${pdEsc(anterior)}</b></div><div>Estado solicitado: <b>${pdEsc(propuesto)}</b></div>`;cont.style.display='block';cont.innerHTML=`<b>Detalle del cambio pendiente</b><div class="pd-item" style="margin-top:8px"><strong>${pdEsc(x.cuadrilla)} · ${pdEsc(x.sede||'')}</strong>${cambioHtml}<div>Motivo: ${pdEsc(x.comentarioSupervisor||x.motivoSolicitud||'Sin comentario')}</div><div>Solicitado por: ${pdEsc(x.solicitadoPor||x.validadoSupervisorPor||'')}</div><div>Estado: <span class="pd-status pendiente">${pdEsc(x.estadoValidacion||x.estadoProgramacion)}</span></div>${u.perfil!=='SUPERVISOR'&&!pdSoloLectura()?`<div class="pd-actions"><button class="pd-btn pd-green" onclick="pdResolverProgramacion('${pdEsc(x.id)}','APROBADO')">Aprobar</button>${esSolicitud?'':`<button class="pd-btn pd-orange" onclick="pdResolverProgramacion('${pdEsc(x.id)}','OBSERVADO')">Observar</button>`}<button class="pd-btn pd-red" onclick="pdResolverProgramacion('${pdEsc(x.id)}','RECHAZADO')">Rechazar</button></div>`:''}</div>`;cont.scrollIntoView({behavior:'smooth',block:'center'});}
function pdHistorialCard(x){const fh=typeof formatearFechaHoraPeruApp==='function'?formatearFechaHoraPeruApp(x.fechaRegistro||'',x.horaRegistro||'',false):`${x.fechaRegistro||''} ${x.horaRegistro||''}`;return `<div class="pd-item"><strong>${pdEsc(x.cuadrilla||'')} · ${pdEsc(x.fechaAfectada||x.fecha||'')}</strong><div>${pdEsc(x.accion||'CAMBIO')} · ${pdEsc(x.estadoAnterior||'')} → ${pdEsc(x.estadoNuevo||'')}</div><div class="pd-note">${pdEsc(x.motivo||'Sin comentario')}</div><div class="pd-note">${pdEsc(x.usuario||'')} · ${pdEsc(fh)}</div></div>`;}
