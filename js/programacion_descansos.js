// MI VISUAL - Programación de Descansos V127
...
function pdRenderTecnico(){
  ...
  <div class="pd-tech-state ${estado==="DESCANSO"?'descanso':'campo'}" style="${estado==='VACACIONES'?'background:#ede9fe;border:3px solid #8b5cf6;color:#6d28d9':''}"><div>ESTADO DE HOY</div><h2>${estado==='VACACIONES'?'🏖️ VACACIONES':(estado==="DESCANSO"?'😴 DESCANSO':'🟢 EN CAMPO')}</h2><div>${pdEsc(new Date(hoy+'T12:00:00').toLocaleDateString('es-PE',{weekday:'long',day:'2-digit',month:'long'}))}</div></div>
...
function pdRenderConsultaOperativa(){
  ...
  const f=pdCapturarFiltros(),fechas=pdFechasVista(),cuadrillas=pdCuadrillasFiltradas(),u=pdUser();
  if(fechas.length===1){
    const fecha=fechas[0],campo=[],descanso=[],vacaciones=[];
    cuadrillas.forEach(c=>{const estado=PD_CAMBIOS[c.cuadrilla+'|'+fecha]||pdEstadoVisible(pdBuscar(c.cuadrilla,fecha));if(estado==='DESCANSO')descanso.push(c);else if(estado==='VACACIONES')vacaciones.push(c);else campo.push(c);});
    ...
    const puedeObservar=['SUPERVISOR','JEFATURA','ADMIN','ADMINISTRADOR'].includes(u.perfil);
    const obs=(PD_DATA.historial||[]).filter(x=>pdNorm(x.accion)==='OBSERVACION OPERATIVA'&&(x.fechaAfectada||x.fecha)===fecha);
    cont.innerHTML=`<b>Consulta operativa del ${pdEsc(fecha)}</b><div class="pd-summary" style="margin-top:8px"><div class="pd-kpi verde"><small>EN CAMPO</small><b>${campo.length}</b></div><div class="pd-kpi"><small>DESCANSO</small><b>${descanso.length}</b></div><div class="pd-kpi" style="background:#ede9fe;border-color:#8b5cf6;color:#6d28d9"><small>VACACIONES</small><b>${vacaciones.length}</b></div></div><div class="pd-query-list" style="margin-top:8px"><div class="pd-query-box"><h4>🟢 Cuadrillas en campo</h4>${lista(campo)}</div><div class="pd-query-box"><h4>😴 Cuadrillas en descanso</h4>${lista(descanso)}</div><div class="pd-query-box"><h4>🏖️ Cuadrillas en vacaciones</h4>${lista(vacaciones)}</div></div>${puedeObservar?`<div class="pd-request"><b>Observación operativa del día</b><div class="pd-note">Registre novedades como cuadrilla programada con un solo técnico, reducción o reasignación de carga.</div><div class="pd-toolbar" style="margin-top:8px"><div class="pd-field"><label>Cuadrilla</label><select id="pdObsCuadrilla">${cuadrillas.map(c=>`<option value="${pdEsc(c.cuadrilla)}">${pdEsc(c.cuadrilla)}</option>`).join('')}</select></div><div class="pd-field"><label>Técnicos disponibles</label><input id="pdObsTecnicos" type="number" min="0" max="2" value="1" style="width:85px"></div><div class="pd-field"><label>Carga / órdenes</label><input id="pdObsCarga" type="number" min="0" value="0" style="width:90px"></div><div class="pd-field"><label>Cuadrilla destino</label><select id="pdObsDestino"><option value="">Sin reasignación</option>${cuadrillas.map(c=>`<option value="${pdEsc(c.cuadrilla)}">${pdEsc(c.cuadrilla)}</option>`).join('')}</select></div><div class="pd-field" style="flex:1;min-width:220px"><label>Observación</label><input id="pdObsComentario" type="text" placeholder="Ej.: cuadrilla programada, pero solo 1 técnico" style="width:100%"></div><button class="pd-btn pd-blue" onclick="pdGuardarObservacionOperativa('${fecha}')">Guardar observación</button></div>${obs.length?`<div class="pd-list" style="margin-top:8px">${obs.map(pdObservacionCard).join('')}</div>`:''}</div>`:''}`;
    ...
    const vacaciones=fechas.filter(fecha=>(PD_CAMBIOS[f.cuadrilla+'|'+fecha]||pdEstadoVisible(pdBuscar(f.cuadrilla,fecha)))==='VACACIONES');
    cont.innerHTML=`<b>Programación de ${pdEsc(f.cuadrilla)}</b><div class="pd-query-box" style="margin-top:8px"><div><b>Descansos:</b> ${descansos.length?descansos.map(x=>`<span class="pd-status descanso" style="display:inline-block;margin:3px">${pdEsc(x)}</span>`).join(''):'<span class="pd-note">Sin descansos.</span>'}</div><div style="margin-top:6px"><b>Vacaciones:</b> ${vacaciones.length?vacaciones.map(x=>`<span class="pd-status vacaciones" style="display:inline-block;margin:3px">${pdEsc(x)}</span>`).join(''):'<span class="pd-note">Sin vacaciones.</span>'}</div></div>`;
  }else cont.innerHTML='<b>Consulta rápida</b><div class="pd-note" style="margin-top:5px">Seleccione <b>Un día</b> para ver quiénes están en campo, descanso o vacaciones, o seleccione una <b>cuadrilla</b> para revisar sus fechas.</div>';
}

function pdObservacionDatos(x){try{return JSON.parse(x.motivo||'{}')}catch(e){return{comentario:x.motivo||''}}}
function pdObservacionCard(x){const d=pdObservacionDatos(x);return `<div class="pd-item"><strong>${pdEsc(x.cuadrilla)} · ${pdEsc(x.fechaAfectada||'')}</strong><div>${pdEsc(d.comentario||'Sin observación')}</div><div class="pd-note">Técnicos disponibles: ${pdEsc(d.tecnicosDisponibles??'')} · Carga: ${pdEsc(d.carga??0)}${d.cuadrillaDestino?' · Reasignar a: '+pdEsc(d.cuadrillaDestino):''}</div><div class="pd-note">${pdEsc(x.usuario||'')} · ${pdEsc(x.fechaRegistro||'')} ${pdEsc(x.horaRegistro||'')}</div></div>`}
async function pdGuardarObservacionOperativa(fecha){try{const u=pdUser(),cuadrilla=document.getElementById('pdObsCuadrilla')?.value||'',tecnicosDisponibles=Number(document.getElementById('pdObsTecnicos')?.value||0),carga=Number(document.getElementById('pdObsCarga')?.value||0),cuadrillaDestino=document.getElementById('pdObsDestino')?.value||'',comentario=(document.getElementById('pdObsComentario')?.value||'').trim();if(!cuadrilla||!comentario)return alert('Seleccione la cuadrilla e ingrese la observación.');await pdApi({accion:'registrarObservacionOperativaDescansos',usuario:u.usuario,fecha,cuadrilla,tecnicosDisponibles,carga,cuadrillaDestino,comentario});alert('Observación operativa registrada.');const per=document.getElementById('pdPeriodo')?.value||PD_DATA.periodo;await pdCargar(per);pdRenderGestion();}catch(e){alert(e.message)}}

function pdToggleDia(cuadrilla,fecha,btn){const key=cuadrilla+'|'+fecha,current=PD_CAMBIOS[key]||pdEstadoVisible(pdBuscar(cuadrilla,fecha)),next=current==='EN CAMPO'?'DESCANSO':(current==='DESCANSO'?'VACACIONES':'EN CAMPO'),base=pdEstadoVisible(pdBuscar(cuadrilla,fecha));if(next===base)delete PD_CAMBIOS[key];else PD_CAMBIOS[key]=next;btn.textContent=next==='DESCANSO'?'D':(next==='VACACIONES'?'V':'C');btn.classList.toggle('pd-descanso',next==='DESCANSO');btn.classList.toggle('pd-vacaciones',next==='VACACIONES');btn.classList.toggle('pd-campo',next==='EN CAMPO');btn.classList.remove('pd-pendiente');pdActualizarBotonCambios();pdCargarCobertura();pdRenderConsultaOperativa();}
...
async function pdCargarCobertura(){
  try{
    ...
    function evaluar(plataforma,dia){
      ...
      const campo=qs.filter(c=>{const e=PD_CAMBIOS[c.cuadrilla+'|'+dia]||pdEstadoVisible(pdBuscar(c.cuadrilla,dia));return e==='EN CAMPO'}).length;
      ...
      if(esSupervisor&&total===3){
        ...
        return{plataforma,total,vacaciones,campo,porcentaje,estado,aplica:false,tipo:'TRES',objetivo:3,minimo:2,objetivoPct:Math.round(regla.objetivo*100),alerta:estado!=='verde'};
      }
      if(esSupervisor&&total===4){
        const estado=campo>=4?'verde':(campo===3?'amarillo':'rojo');
        return{plataforma,total,vacaciones,campo,porcentaje,estado,aplica:false,tipo:'CUATRO',objetivo:4,minimo:3,objetivoPct:Math.round(regla.objetivo*100),alerta:estado!=='verde'};
      }
