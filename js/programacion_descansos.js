...
function pdRenderGestion(){
  ...
  const historial=(PD_DATA.historial||[]).filter(x=>esSupervisor?pdNorm(x.sede)===u.sede:true).slice(0,80);
  ...
  <div class="pd-card"><b>${esSupervisor?'Solicitudes de técnicos pendientes de Supervisor':'Cambios pendientes de Jefatura'}</b><div class="pd-list" style="margin-top:8px">${pendientes.length?pendientes.map(pdSolicitudCard).join(''):'<div class="pd-note">No hay solicitudes pendientes.</div>'}</div></div>
...
function pdToggleDia(cuadrilla,fecha,btn){const key=cuadrilla+'|'+fecha,current=PD_CAMBIOS[key]||pdEstadoVisible(pdBuscar(cuadrilla,fecha)),next=current==='DESCANSO'?'EN CAMPO':'DESCANSO';const base=pdEstadoVisible(pdBuscar(cuadrilla,fecha));if(next===base)delete PD_CAMBIOS[key];else PD_CAMBIOS[key]=next;btn.textContent=next==='DESCANSO'?'D':'C';btn.classList.toggle('pd-descanso',next==='DESCANSO');btn.classList.toggle('pd-campo',next!=='DESCANSO');btn.classList.remove('pd-pendiente');pdActualizarBotonCambios();pdCargarCobertura();pdRenderConsultaOperativa();}
...
async function pdCargarCobertura(){
  try{
    ...
    function evaluar(plataforma,dia){
      ...
      const campo=qs.filter(c=>(PD_CAMBIOS[c.cuadrilla+'|'+dia]||pdEstadoVisible(pdBuscar(c.cuadrilla,dia)))!=='DESCANSO').length;
      const porcentaje=campo/total,regla=pdReglaCobertura(plataforma,dia);

      if(esSupervisor&&total===1){
        return{plataforma,total,campo,porcentaje,estado:'verde',aplica:false,tipo:'UNA',objetivoPct:Math.round(regla.objetivo*100),alerta:false};
      }
...
function pdMostrarDetallePendiente(id){const x=PD_DATA.programacion.find(i=>i.id===id);const cont=document.getElementById('pdDetallePendiente');if(!x||!cont)return;const u=pdUser(),anterior=pdNorm(x.estadoDia||'EN CAMPO'),propuesto=pdEstadoPropuesto(x);cont.style.display='block';cont.innerHTML=`<b>Detalle del cambio pendiente</b><div class="pd-item" style="margin-top:8px"><strong>${pdEsc(x.cuadrilla)} · ${pdEsc(x.fecha)}</strong><div>Estado vigente: <b>${pdEsc(anterior)}</b></div><div>Estado solicitado: <b>${pdEsc(propuesto)}</b></div><div>Motivo: ${pdEsc(x.motivoSolicitud||'Sin comentario')}</div><div>Solicitado por: ${pdEsc(x.solicitadoPor||'')}</div><div>Estado: <span class="pd-status pendiente">${pdEsc(x.estadoProgramacion)}</span></div>${u.perfil!=='SUPERVISOR'?`<div class="pd-actions"><button class="pd-btn pd-green" onclick="pdResolverProgramacion('${pdEsc(x.id)}','APROBADO')">Aprobar</button><button class="pd-btn pd-orange" onclick="pdResolverProgramacion('${pdEsc(x.id)}','OBSERVADO')">Observar</button><button class="pd-btn pd-red" onclick="pdResolverProgramacion('${pdEsc(x.id)}','RECHAZADO')">Rechazar</button></div>`:''}</div>`;cont.scrollIntoView({behavior:'smooth',block:'center'});}
