// MI VISUAL - Programación de Descansos V111
...
function pdRenderCalendario(){
  ...
  let mesAnt='';fechas.forEach(fecha=>{const mes=fecha.slice(0,7),cambio=mes!==mesAnt,domingo=pdDiaCorto(fecha)==='D';html+=`<th class="${cambio?'pd-month-label ':''}${domingo?'pd-sunday':''}">${cambio?pdNombreMes(mes).split(' ')[0]+'<br>':''}${pdDiaCorto(fecha)}<br>${Number(fecha.slice(8,10))}</th>`;mesAnt=mes;});html+='</tr></thead><tbody>';
  ordenadas.forEach(c=>{
