/* ============================================================
   MI VISUAL V440 - SEGURIDAD ATS / PETAR DIGITAL - INGRESO GUIADO
...
const LOGO_EMPRESA='./img/logo-visual-connections.png?v=V440';
...
let SEG_GUIA={paso:0,tarea:0,petar:0,confirmadas:{}};
...
function cacheKey(){return 'MI_VISUAL_SEG440_CTX_'+N(user())}
function cacheLeer(){
  try{
...
async function abrirAtsCore(id){
  ...
  if(perfil()==='TECNICO'&&['BORRADOR','OBSERVADO'].includes(N(SEG.ats?.estado))){SEG_GUIA={paso:0,tarea:0,petar:0,confirmadas:{}}}
...
function guiaNormalizarTarea(t){
  const c=guiaCatalogoTarea(t?.tarea);
  return {tarea:n(t?.tarea),danos:Array.isArray(t?.danos)?[...t.danos]:(c?.danos||[]).slice(),otrosDano:n(t?.otrosDano),controles:n(t?.controles)||(c?.controles||[]).join('\n')};
}
...
function guiaCapturarTareas(){
  ...
  SEG.ats.tareas=nombres.map(nombre=>{
    const viejo=prev.get(N(nombre));if(viejo)return guiaNormalizarTarea(viejo);
    const c=guiaCatalogoTarea(nombre);return {tarea:nombre,danos:(c?.danos||[]).slice(),otrosDano:'',controles:(c?.controles||[]).join('\n')};
  });
...
function guiaCapturarRiesgosControles(){
  const t=SEG.ats.tareas?.[SEG_GUIA.tarea];if(!t)return false;
  ...
  const conf=!!document.getElementById('segGuiaConfirmo')?.checked,otroOk=!t.danos.some(x=>N(x)==='OTROS')||!!t.otrosDano;
  if(t.danos.length&&controles.length&&conf&&otroOk)SEG_GUIA.confirmadas[N(t.tarea)]=true;else delete SEG_GUIA.confirmadas[N(t.tarea)];
  return t.danos.length>0&&controles.length>0&&conf&&otroOk;
}
...
function seg440TareaSiguiente(){if(!guiaCapturarRiesgosControles())return alert('Seleccione al menos un posible daño, una medida de control y confirme que las ha leído.');if(SEG_GUIA.tarea<(SEG.ats.tareas||[]).length-1)SEG_GUIA.tarea++;else{SEG_GUIA.paso=5;SEG_GUIA.petar=0}renderTecnicoGuiado()}
