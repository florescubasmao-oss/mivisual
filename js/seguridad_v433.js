/* ============================================================
   MI VISUAL V433 - SEGURIDAD ATS / PETAR DIGITAL
   Ajuste incremental sobre V432.
...
const LOGO_EMPRESA='./img/logo-visual-connections.png?v=V433';
...
function cacheKey(){return 'MI_VISUAL_SEG433_CTX_'+N(user())}
function cacheLeer(){
  try{
...
.seg-table{width:100%;border-collapse:collapse;min-width:900px;font-size:10px}.seg-table th,.seg-table td{border:1px solid #64748b;padding:5px;vertical-align:middle}
.seg-table th{background:#f1f5f9}.seg-table input,.seg-table select,.seg-table textarea{width:100%;box-sizing:border-box;background:#fff;font:inherit}
...
.seg-task-select{min-width:185px}.seg-danos{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3px}
.seg-danos label{font-size:8.5px;display:flex;gap:3px;align-items:flex-start}.seg-sign{max-height:50px;max-width:140px}
...
.seg-tools-box{min-width:300px}.seg-tools-search{border:1px solid #94a3b8!important;border-radius:8px!important;padding:7px!important;margin-bottom:6px}
.seg-tools-meta{display:flex;justify-content:space-between;gap:8px;align-items:center;font-size:9px;font-weight:800;color:#475569;margin-bottom:5px}
.seg-tool-groups{max-height:235px;overflow:auto;border:1px solid #cbd5e1;border-radius:8px;padding:5px;background:#fff}
.seg-tool-group{margin-bottom:6px}.seg-tool-group:last-child{margin-bottom:0}.seg-tool-group-title{position:sticky;top:-5px;background:#e2e8f0;padding:4px 6px;border-radius:6px;font-size:9px;font-weight:900;color:#334155;z-index:1}
.seg-tool-items{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3px;margin-top:4px}
.seg-tool-item{display:flex;gap:4px;align-items:flex-start;border:1px solid #e2e8f0;border-radius:6px;padding:4px;font-size:8.5px;background:#f8fafc}
...
function home(){
  ...
  if(tec){
    const hay=!!c.hoy?.ats;
    html+=`<div class="seg-card"><b>ATS / PETAR del día</b><div class="seg-note" style="margin-top:7px">${hay?`ATS N° ${e(c.hoy.ats.numero)} · ${e(c.hoy.ats.estado)}`:'Aún no se ha generado el ATS de hoy para la cuadrilla.'}</div><div class="seg-actions">${hay?`<button class="seg-btn green" onclick="seg433AbrirAts('${e(c.hoy.ats.id)}')">Abrir ATS/PETAR</button>`:`<button class="seg-btn green" onclick="seg433GenerarDia(this)">Generar ATS/PETAR del día</button>`}</div></div>`;
  }else{
    const pend=c.pendientes||[];
    const pendHtml=pend.length?pend.map(x=>`<div class="seg-item"><b>ATS N° ${e(x.numero)} · ${e(x.cuadrilla)}</b><small style="display:block">${e(fmtFecha(x.fecha))} · ${e(x.sede)} · ${e(x.estado)}</small><button class="seg-btn" style="margin-top:6px" onclick="seg433AbrirAts('${e(x.id)}')">Revisar</button></div>`).join(''):'<div class="seg-note seg-ok">Sin pendientes.</div>';
    html+=`<div class="seg-card"><b>Pendientes de revisión</b><div class="seg-pend" style="margin-top:8px">${pendHtml}</div></div>`;
...
function toolsHtml(editable){
  ...
  const cuerpo=Object.keys(grupos).sort((a,b)=>a.localeCompare(b)).map(g=>`<div class="seg-tool-group" data-tool-group="${e(N(g))}"><div class="seg-tool-group-title">${e(g)}</div><div class="seg-tool-items">${grupos[g].map(x=>`<label class="seg-tool-item" data-tool-item="${e(N(x.herramienta))}"><input type="checkbox" data-tool="${e(x.herramienta)}" ${sel.has(N(x.herramienta))?'checked':''} onchange="seg433ToolToggle()"> <span>${e(x.herramienta)}</span></label>`).join('')}</div></div>`).join('');
  return `<div class="seg-tools-box"><input class="seg-tools-search" id="segToolSearch" placeholder="Buscar herramienta..." oninput="seg433FiltrarHerramientas(this.value)"><div class="seg-tools-meta"><span>Seleccione las herramientas utilizadas</span><b id="segToolCount">${sel.size} seleccionada(s)</b></div><div class="seg-tool-groups">${cuerpo}</div></div>`;
}
function toolToggle(){
...
function filtrarHerramientas(valor){
  const q=N(valor);
  document.querySelectorAll('[data-tool-item]').forEach(el=>el.style.display=!q||N(el.dataset.toolItem).includes(q)?'flex':'none');
  document.querySelectorAll('[data-tool-group]').forEach(g=>{
    const visible=[...g.querySelectorAll('[data-tool-item]')].some(x=>x.style.display!=='none');
...
function taskRow(t,i,editable){
  ...
  return `<tr data-task="${i}">
    ...
    <td><div class="seg-danos">${DANOS.map(d=>`<label><input type="checkbox" data-d="${e(d)}" ${(t.danos||[]).some(x=>N(x)===N(d))?'checked':''} ${editable?'':'disabled'}> ${e(d)}</label>`).join('')}</div></td>
...
function renderAts(){
  ...
  <div class="seg-paper" style="margin-top:8px">
    <div class="seg-doc-head">
      ...
      <div><div>ATS N° <span class="seg-nro">${e(a.numero)}</span><br><small>SGSST_ATS_01<br>Versión: ${e(a.version||1)}</small></div></div>
    </div>
    <div class="seg-meta">
      <div class="seg-grid">
    ...
    <table class="seg-table">
      ...
      <tr>
        <td colspan="4"><div class="seg-checks">${(SEG.catalogo?.epp||[]).map(x=>`<label class="seg-check"><input type="checkbox" data-epp="${e(x)}" ${(a.epp||[]).includes(x)?'checked':''} ${editable?'':'disabled'}>${e(x)}</label>`).join('')}</div></td>
...
function renderPetar(editable){
  ...
  <div class="seg-paper">
    <div class="seg-doc-head"><div><img src="${LOGO_EMPRESA}" alt="Visual Connections"></div><div class="seg-doc-title">PERMISO ESCRITO PARA TRABAJOS DE ALTO RIESGO (PETAR) - ALTURA</div><div>PETAR N° <span class="seg-nro">${e(p.numero)}</span></div></div>
    ...
    <div style="margin-top:8px"><b>EQUIPO DE PROTECCIÓN REQUERIDO</b><div class="seg-checks">${(SEG.catalogo?.eppPetar||[]).map(x=>`<label class="seg-check"><input type="checkbox" data-pepp="${e(x)}" ${(p.epp||[]).includes(x)?'checked':''} ${editable?'':'disabled'}>${e(x)}</label>`).join('')}</div></div>
  </div>`;
}
...
function datosAts(){
  const tareas=[...document.querySelectorAll('#segTaskTable tbody tr')].map(tr=>{
    ...
    return {tarea,danos:[...tr.querySelectorAll('[data-d]:checked')].map(x=>x.dataset.d),controles:n(tr.querySelector('[data-k="controles"]')?.value)};
  }).filter(x=>x.tarea);
  return {
    ...
    epp:[...document.querySelectorAll('[data-epp]:checked')].map(x=>x.dataset.epp),
    tareas
  };
}
function datosPetar(){
  if(!SEG.petar)return null;
  return {checklist:(SEG.petar.checklist||[]).map((x,i)=>Object.assign({},x,{estado:n(document.querySelector(`[data-petar-i="${i}"]`)?.value),observacion:n(document.querySelector(`[data-petar-o="${i}"]`)?.value)})),epp:[...document.querySelectorAll('[data-pepp]:checked')].map(x=>x.dataset.pepp)};
}
...
async function guardar(reabrir=true){
  try{
    ...
    cacheBorrar();
    if(reabrir)await abrirAts(SEG.ats.id);
    return true;
...
async function aceptar(){
  const ok=await guardar(false);if(!ok)return;
  try{
    const g=await geo();
    await api({accion:'aceptarAtsSeguridadV432',id:SEG.ats.id,gps:g});
    cacheBorrar();await abrirAts(SEG.ats.id);
  }catch(err){alert(err.message)}
}
async function revisar(accion){
  const motivo=accion==='AUTORIZAR'?'':prompt('Motivo:')||'';
  if(accion!=='AUTORIZAR'&&!motivo)return;
  try{await api({accion:'revisarAtsSupervisorV432',id:SEG.ats.id,resultado:accion,motivo});cacheBorrar();await abrirAts(SEG.ats.id)}catch(err){alert(err.message)}
}
async function validarFinal(resultado){
  ...
  try{
    ...
    cacheBorrar();if(d.pdfUrl)alert('Documento cerrado y PDF generado.');await abrirAts(SEG.ats.id);
  }catch(err){alert(err.message)}
}
...
async function solicitarCambioFirma(){
  ...
  try{await api({accion:'solicitarCambioFirmaSeguridadV432',motivo});cacheBorrar();alert('Solicitud enviada.');await cargar(true)}catch(err){alert(err.message)}
}
async function resolverFirma(id,resultado){
  try{await api({accion:'resolverCambioFirmaSeguridadV432',id,resultado});cacheBorrar();await cargar(true)}catch(err){alert(err.message)}
}
async function reiniciarFirmaPruebas(){
  ...
  try{
    const d=await api({accion:'reiniciarFirmaSeguridadV433',usuarioObjetivo:objetivo});
    cacheBorrar();alert(`Firma reiniciada. Registros eliminados: ${d.borradas||0}.`);await cargar(true);
  }catch(err){alert(err.message)}
}
